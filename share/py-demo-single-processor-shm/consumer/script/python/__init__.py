"""Throughput consumer: reports slots/s, MB/s, wait/work times, and correctness every second.
...
_window_slots: int = 0
_window_bytes: int = 0
_window_wait_us: int = 0    # sum of last_slot_wait_us samples in window
_window_work_us: int = 0    # sum of last_slot_work_us samples in window
...
def on_consume(rx, messages, api: cons.ConsumerAPI) -> None:
    ...
    # Sample queue-depth proxy metrics from the SHM driver.
    m = api.metrics()
