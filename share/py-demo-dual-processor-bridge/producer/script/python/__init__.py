"""Dual-hub demo producer: emits count + ts + 1024 float32 samples at max rate.

Slot: count(int64), ts(float64), samples(float32×1024)
...
def on_init(api: prod.ProducerAPI) -> None:
    ...
    api.log(
        "info",
        f"BridgeProducer: started uid={api.uid()} "
...
def on_stop(api: prod.ProducerAPI) -> None:
    ...
    api.log(
        "info",
        f"BridgeProducer: stopped total={_count} avg_rate={_count/elapsed:.1f} Hz "
