...
| | |
|---|---|
| **Status** | IMPLEMENTED — 2026-07-26 (one coherent S1+S2+S3 unit + test migration, per the integration-order finding below).  Deltas from plan, discovered at code time: (1) S1 got a SECOND layer — `_on_producer_added` refuses a fan-in fresh-channel open under the writer lock (`topology_error_code="AWAITING_OWNER"`), closing the snapshot→admission race the side-effect-free handler gate cannot; (2) the consumer raced-admission reply (`admitted=false` silent-skip, "vanished during admission") also became AWAITING_OWNER — wire reply must be a pure function of state, not lock timing; (3) S2 needed broker close-out wiring at TWO sites (`handle_consumer_dereg_req` + the heartbeat reconciler's consumer branch) and `_on_consumer_left` / `_remove_consumer` now return results; (4) S3 was verified ALREADY DONE role-side (`BrcOracle::poll` maps any broker ERROR — incl. CHANNEL_NOT_FOUND — to PermanentError; `finalize_connect` aborts immediately on it) — no code change, L4 pin only.  Retry loops live in `RoleAPIBase::register_producer_channel` / `register_consumer` (extending the existing CHANNEL_NOT_READY loop), signature-shaped like `finalize_channel_connect` (`timeout_ms` = total budget + `is_cancelled`); all four role-host call sites pass `init_timeout_ms` + the shutdown predicate. |
