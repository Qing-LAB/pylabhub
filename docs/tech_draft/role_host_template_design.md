...
| **M4** ⏳ in progress | `RoleAPIBase::pImpl` swap.  All 24 `pImpl->broker_channel` sites delegate via handler.  Sub-shipped leaf-first per user direction:  **M4a ✅** (2026-05-15) ships `RoleHandler::start_connections(owner)` / `stop_connections()` — STATE-only.  Allocates BRC per HubConnection + connects DEALER socket.  NO thread spawning by the handler (RoleHandler is a state holder + routing helper; threads stay with RoleHost per the action-taker/state-holder separation).  After M4a, M4a's BRCs are **inert** — connected but no poll loop drives them.  See HEP-CORE-0031 §4.3.6 for the M4 thread transition plan.  **M4b** (next) adds `send_class_A/B/C/D` dispatch methods.  **M4c** extends `RoleAPIBase::start_ctrl_thread` to spawn one ctrl thread per `handler.connections()` (where the inert BRCs become live).  **M4d/e** migrate the 24 call sites by routing class.  **M4f** removes `pImpl->broker_channel`. | **SHIPPED M4a**: 3 L3 tests (`test_datahub_role_state_machine.cpp`, Pattern 3 IsolatedProcessTest) — `RoleHandler_Connections_StartStop_Smoke` (single producer), `RoleHandler_Connections_DualHub` (M8-payoff data shape), `RoleHandler_Connections_DoubleStart_Rejected` (idempotency).  All state-only — no thread assertions.  **REVISED (M4d/e)**: every test setting up a `RoleAPIBase` now creates it with a `RoleHandler` (test fixture).  Public API surface unchanged so existing tests keep passing.  **NEW (M4c/d)**: `RoleAPIBaseTest::HandlerDelegation_SingleConnection_BehaviourPreserved` — explicit confirmation. | B3 |
