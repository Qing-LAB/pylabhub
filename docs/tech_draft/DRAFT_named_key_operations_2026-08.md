...
| 5 | re-express the vault tests off the secret accessors ✅ **DONE** (`ce8c67e3`) | — **ORDERING WAS WRONG IN THIS PLAN:** six of the sites compare the SECRET across create/open/save, so they needed `load_identity_into` to exist before they could move.  Built it first, then migrated, then deleted.  Also: the proposed pubkey-comparison substitute would have WEAKENED them — a bug corrupting only the secret passes it.  Used a BLAKE2b fingerprint taken inside `with_seckey_z85` instead. |
| 6 | `load_identity_into` ✅ **DONE**; secret accessors ✅ **DELETED**; two production couriers ✅ **REMOVED** — but `create` restructure and the `secret_z85` members ⬜ **REMAIN** | **the couriers (§3.3) — CLOSED.**  The four-copy chain (§3.2) and the unlocked member (§3.4) are NOT closed: deleting the accessor stopped the secret LEAVING, not BEING there.  Closing them means `open()` depositing during parse with no member at all, and `create` minting via `generate_and_add_identity`.  **This is the step that changes what is inside the payload — the only one so far that needs a migration story.** |
| 7 | make the raw-key `secretbox_*` private; decide `lookup_raw` ✅ **DONE (`072787e6`) — DELETED, not privatised** | **the ability to reintroduce any of it — CLOSED.**  🟢 **Precondition met (`2fbc54f7`): both surfaces now have ZERO production callers.**  What remains is test-side — the L2 workers still exercise both directly, so step 5's re-expression has to land before the doors can close. |
