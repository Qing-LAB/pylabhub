...
| `DatahubE2ETest.ConsumerHandlesLateStart` | consumer attaches AFTER producer-started | name lookup retry on `/dev/shm` | broker mediates attach via `CONSUMER_ATTACH_REQ_SHM` — broker holds producer's transport; late consumers ask broker | **(a') L4 e2e (#258)** (was planned as `Pattern4DataPipelineTest.LateConsumerAttachViaBroker`) |
