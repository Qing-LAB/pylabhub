...
| **3** | ✅ DONE 2026-05-10 (commits `9f3ddcb` step 3a + `ed15f02` step 3b).  Rewrote REG_REQ handler to use `add_producer` + per-producer setters via new `HubState::_on_producer_added` op.  Eliminated the build-fresh-entry-and-replace-channel pattern.  Typed `ProducerAdmissionResult` dispatches `UID_CONFLICT` / `MULTI_PRODUCER_NOT_SUPPORTED_FOR_SHM` / `SCHEMA_MISMATCH` / `TRANSPORT_MISMATCH` wire codes.  DISC_REQ_ACK metadata echo migrated to tree shape; endpoint + pubkey echoes use first-producer transitional shape until step 5.  Tests: 5 admission scenarios via `HubStateTestAccess::on_producer_added`; 3 pre-MP2.5 same-uid restart tests updated to assert new strict reject. | `broker_service.cpp:handle_reg_req`, `hub_state.cpp:_on_producer_added` | L2: 5 admission scenarios + updated re-register tests; full suite 1790/1790. |
...
| **7** | Make state-bearing fields private (or move into `Impl`).  Compile-fail any remaining direct field write.  **Trigger: a concrete bug surfaces from a new direct-access site.**  The overwrite-class bug class is already structurally eliminated by Wave M2.5 steps 1-6.5 (no channel-scope per-party fields remain).  Step 7 is polish that hardens the wall against future contributors; no calendar trigger.  Bug-driven only.  Tracked in TODO_MASTER §"Deferred items — explicit phase entries". | `hub_state.hpp` | Build must remain clean. |
| **2d** | Deferred API methods from §5.1 — `set_invariant_schema` / `set_invariant_transport` (need field-by-field invariant precision design pass; broker's Cat-1 schema check owns this contract for now), `observable(roles_map)` member wrapper (sugar over the free `compute_channel_observable`), `is_alive(roles_map)`, `producers()` / `consumers()` span accessors.  **Trigger: first concrete caller need.**  These are sugar; no current production code asks for them.  Caller-driven, not calendar-driven.  Tracked in TODO_MASTER §"Deferred items — explicit phase entries". | `src/include/utils/hub_state.hpp` | TBD |
| **8** | HEP doc sweep.  **Mostly DONE inline during steps 3-6.5** — see TODO_MASTER §"Step 8 audit — what's shipped vs remaining" for the line-item breakdown.  **Remaining:** HEP-CORE-0019 §9 update for the per-producer `producer_metrics` tree shape introduced by G1 (commit `416cbec`); small cross-references between HEPs.  **Trigger: alongside HEP-0019's next edit, or as a standalone small commit when convenient.**  No urgency — code is the truth; HEP-0019 lags. | `docs/HEP/HEP-CORE-0019.md` | — |
