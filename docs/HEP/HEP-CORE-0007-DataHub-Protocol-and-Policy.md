...
Gating:     `role_uid` REQUIRED on every SCHEMA_REQ — it is the CALLER's
            ...
            (Control_EnvelopeWithRoleUid — the caller's own identity-checked uid).
...
Gating:     `channel_name` REQUIRED — the all-channels wire form is
            RETIRED (2026-07-26): hub-wide aggregation stays on
...
query answers from state and never waits.
```
...
| `SCHEMA_REQUIRED` | CONSUMER_REG_REQ that would OPEN a fan-in channel with NO schema material.  The owner establishes the channel's format when it opens the book (HEP-CORE-0017 §4.7.0.1 C1/C2; ratified 2026-07-26): a blank contract would then reject every schema-carrying producer under the exact-equality matcher.  Joining consumers may still register uncited (opt-out mode is a JOIN-only right).  **New 2026-07-26.** | Configuration error: give the fan-in owner its schema (inline, file, or hash-pin + structure).  No retry without fixing the config. |
...
| `SCHEMA_FORBIDDEN_OWNER` | REG_REQ / CONSUMER_REG_REQ citing `schema_owner` that is neither `"hub"` nor a `role_uid` of any producer currently admitted on the channel (HEP-CORE-0034 §9.1; multi-producer-aware per HEP-CORE-0023 §2.1.1).  **Also (ruled 2026-07-26): a fan-in channel-OPEN citation claiming any owner other than `"hub"`** — a consumer cannot open under a producer's namespace. | Cite hub-globals or one of the channel's registered producer's schemas only. |
| `SCHEMA_OWNER_REQUIRED` | CONSUMER_REG_REQ that would OPEN a fan-in channel with a NAMED citation (`expected_schema_id` set) but no `expected_schema_owner`.  An unowned named book is unjoinable by every producer: the front door defaults a named producer citation's owner to self, and anonymous joiners fail the name axis — so the named open must designate the hub registry (ruled 2026-07-26). | Configuration error: set `expected_schema_owner="hub"` (and ensure the hub-global exists), or open anonymously with full structure. |
| `SCHEMA_UNKNOWN` | REG_REQ path-C citing `schema_owner="hub"` + `schema_id=X` where the broker has no record for that key.  **Also (ruled 2026-07-26): a fan-in channel-OPEN named citation (owner `"hub"`) whose `(hub, id)` has no registry record** — the open is a registry citation and resolves exactly like path C. | Producer must register the schema (path-B) or wait for the hub-global to be loaded.  Verify schema_search_dirs at hub startup. |
| `SCHEMA_CITATION_REJECTED` | CONSUMER_REG_REQ where the cited schema's stored fingerprint disagrees with the consumer's expected fingerprint (HEP-CORE-0034 §9.3), **or (ruled 2026-07-26) where a joining consumer's non-empty `expected_schema_owner` claim does not equal the channel's stored owner** (exact match on every declared axis; empty = no claim). | Reconcile schemas; do not retry without aligning fingerprints (or the owner claim). |
| `FINGERPRINT_INCONSISTENT` | Any registration whose schema STRUCTURE doesn't hash to its claimed `schema_hash` (broker re-derives and rejects): named producer REG_REQ; anonymous producer REG_REQ carrying structure (open-row rule, added 2026-07-26); consumer citation with structure — joining OR opening a fan-in channel (the owner's citation is validated before the book opens, 2026-07-26). | Programming error / stale config on the REJECTED party's side — the hash and structure in its config disagree.  Fix the config; retry. |
...
| `MISSING_HASH` | Registration structure without its fingerprint: named REG_REQ without `schema_hash`; anonymous REG_REQ carrying `schema_blds` without `schema_hash` (open-row rule, 2026-07-26); fan-in channel-OPEN citation carrying structure without `expected_schema_hash` (the installed contract must carry its fingerprint, 2026-07-26). | Compute and supply the hash; retry. |
