...
| `MISSING_HASH` | Registration structure without its fingerprint: named REG_REQ without `schema_hash`; anonymous REG_REQ carrying `schema_blds` without `schema_hash` (SI-2 open-row rule, 2026-07-26); fan-in channel-OPEN citation carrying structure without `expected_schema_hash` (the installed contract must carry its fingerprint, 2026-07-26). | Compute and supply the hash; retry. |
