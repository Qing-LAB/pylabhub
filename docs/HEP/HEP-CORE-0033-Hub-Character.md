...
|---|---|---|
| `ChannelEntry` | REG_REQ / CONSUMER_REG_REQ / CONSUMER_DEREG_REQ / CHANNEL_CLOSING / broker-internal | name, **schema_owner**, **schema_id**, schema_hash + blds + packing (channel-wide invariant — all producers must agree), **`producers[]`** (vector of `ProducerEntry`, parallel to `consumers[]`, supports 1..N producers per HEP-CORE-0023 §2.1.1), **`consumers[]`** (vector of `ConsumerEntry`), data-plane endpoints, created_at.  **Does NOT store FSM state** — both channel **observability** (HEP-CORE-0023 §2.2) and channel **existence** (HEP-CORE-0023 §2.1.1) are derived queries over the producer-presences across roles.  Channel teardown is atomic on the LAST producer-presence transitioning to Disconnected; no separate channel FSM. |
