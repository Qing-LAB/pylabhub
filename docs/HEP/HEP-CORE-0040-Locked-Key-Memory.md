...
| **Related**     | HEP-CORE-0001 (Hybrid Lifecycle Model) — base contract for registered subsystems. HEP-CORE-0031 (ThreadManager + Thread Shutdown Contract) — dynamic auto-registered module pattern this HEP mirrors. HEP-CORE-0035 §4.6 (key-file storage discipline at rest) + §4.7 (runtime key handling consumer surface). HEP-CORE-0038 (script-accessible vault keystore) — consumer of dynamic KeyStore for runtime-saved scripted secrets. HEP-CORE-0041 (SHM Channel Auth) — sibling security HEP for the SHM data plane.  ACTIVE consumer of this HEP's KeyStore + LockedKey primitives as of Phase 1 substep 1c (shipped 2026-06-17): HEP-0041 §5.5 reads the producer's `kRoleIdentityName` seckey via the `SeckeyAccessor` callback (use-not-export, §8.5.1) to decrypt the consumer's `crypto_box_easy(challenge, consumer_pk, producer_sk)` proof during SHM attach.  Future Phase 4 (Option C encrypted-payload SHM) would extend the use to AEAD/KDF key material; Phase 1's use is identity-key decryption only. |
...
| Wire deserialization (CONSUMER_REG_ACK.producers[].pubkey, REG_ACK.initial_allowlist, GET_CHANNEL_AUTH_ACK.allowlist, federation peer announce) | `try_validate()` | Network-arriving input may be tampered with, corrupted, or sent by a buggy peer.  Throwing from a poll-loop callback would unwind out of the poll thread and crash the process.  The handler logs the bad input, drops the offending message, and keeps the process alive. |
