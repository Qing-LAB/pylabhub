...
|  | | ⏭ | **1i-coverage** (#270): **LAYER PIVOT 2026-06-25 — folded into #258 (L4 end-to-end SHM auth-gated data flow).**  Initial implementation tried L2 coverage via `RoleHostFrameTestShim` in `tests/test_framework/` (subclass of `RoleHostFrame` exposing protected `prepare_tx_capability_` + `spawn_shm_auth_listener_` + `cleanup_tx_capability_` as one-line forwards).  Shipped 2026-06-25 as commits `8716d91a` + `31b1d2af`; reverted same day as commit `<TBD>` after designer review surfaced the layering smell: the shim's `worker_main_` override mirrored production's `teardown_infrastructure_` ordering, making it a **parallel production scaffold** rather than a test of production.  The three methods being tested are setup-step contributors to a multi-process authentication flow, not unit-testable behaviors in isolation; an L4 test using real `plh_hub` + `plh_role` binaries naturally exercises them in their real production context.  Diagnostic visibility preserved via the production log markers (`event=ShmCapabilityTransportBound` at `role_host_frame.cpp:427-431` + `event=ShmAcceptLoopSpawned` at `:643-645`) which #258's L4 test will grep via `expect_log_sequence` against captured stderr.  Lessons captured: (i) deliberate producer-side asymmetry — RoleHostFrame owns SHM transport vs. RoleAPIBase owns consumer dial — is correct architecture, not a half-finished consolidation; (ii) "build a parallel production scaffold to test protected methods" is a test-faithfulness antipattern even when the scaffold is one-line forwards; (iii) for multi-process production code, the natural test unit IS the multi-process scenario.  No follow-up at L2 layer. |
