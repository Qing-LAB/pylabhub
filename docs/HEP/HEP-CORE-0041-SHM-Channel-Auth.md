...
|  | | ✅ | **1i-coverage** (#270): L2 coverage for `RoleHostFrame::spawn_shm_auth_listener_` + `prepare_tx_capability_` + `cleanup_tx_capability_` shipped via `tests/test_layer2_role_host_frame_shm`.  Four Pattern-3 subprocess tests pin: (a) ZMQ TX no-op early return; (b) SHM TX happy path → `IShmCapabilityProducer` bound + `tx_opts.shm_capability_fd` populated; (c) `spawn_shm_auth_listener_` pre-condition violation (null `shm_transport_` → false + ERROR log); (d) prepare → spawn → production teardown sequence (`api().stop_handler_threads()` drains accept thread → `cleanup_tx_capability_` LIFO release) without leaked threads.  Test-side `RoleHostFrameTestShim` in `tests/test_framework/` exposes the three protected methods as one-line public forwards (no behavior change; matches `pattern4_consumer_lifecycle_workers.cpp` test-faithfulness principle).  Reusable by Pattern 4 SHM rungs under #285. |
