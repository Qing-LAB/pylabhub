...
| **Status** | 🟢 **DESIGN FINAL; PHASE 1 IN FLIGHT** — substeps 1a-1h ✅; 1i-mig-1/2a/2b-1/2b-2/2c/M3/3 ✅ (TX side complete on both producer + processor); 1i-mig-M3.5 ✅ (#266 — `prepare_tx_capability_` promoted to RoleHostFrame default; ~92 LOC saved); pre-REVIEW-A cluster #267/#268/#269/#279 ⏸ (#279 added 2026-06-22 from queue-abstraction architectural assessment — last script-visible queue-layer leak); 1i-mig-4 (consumer dial) ⏸; 1i-mig-5 + 1i-cleanup + 1j + 1k + #262 ⏸.  Five REVIEW-A..E milestones gate the remaining chain.  Live tracker at §10.1. |
| **Created** | 2026-06-16 |
...
|  | | ⏸ | **1i-mig-M3.5** (#266): **next.**  Promote `prepare_tx_capability_` to `RoleHostFrame` default impl (currently byte-equivalent in producer + processor, ~70 LOC saved). |
|  | | ⏸ | **1i-doc-sync** (#267): doc-sync bundle — §10.1 status (this row), §5.1 (`strip_unix_scheme` + parent-dir mkdir contract), §6.1 (M3 3-pointer bundle on RoleHostFrame; add L2c → L1.send_capability edge), §6.3 (`borrow_fd` interface), HEP-CORE-0031 §4.3.2/§4.3.5 (`shm_accept_loop` thread inventory row), HEP-CORE-0036 §4.1 (mark `ChannelAccessEntry::shm_secret` as historical-pending-cleanup), §9 D4 diagram rename `CONSUMER_ATTACH_RSP → _ACK` (consistent with prose + code), SO_PEERCRED citation fix (HEP-0036 §I8 is "Trust model"; needs distinct anchor or generic cite), `broker_proto` version-comment decision for HEP-0041 additions, plus document L2-known-limitations (handshake-deadline fused with accept-poll at 100ms, listen backlog=8). |
|  | | ⏸ | **1i-prod-hardening** (#268): producer-side hardening — H3a defer accept-thread spawn until after `set_value(true)` (avoids admitting consumers to non-live role); H3b call `teardown_infrastructure_` on `apply_producer_reg_ack` failure (closes L1 socket); H3c broker REG_REQ rejects empty `shm_capability_endpoint` for SHM channels; H3d `bind_endpoint` probe-then-unlink to avoid silent steal of a live producer's socket. |
|  | | ⏸ | **1i-api-scope** (#269): `RoleAPIBase::consumer_attach` is currently public → script-binding leak risk.  Verify engine bindings (Lua/Python/Native), then make internal (move to `Impl` accessor) OR annotate `SCRIPT_PRIVATE`. |
|  | | ⏸ | **Mechanism enum widening** (#279): `hub::Mechanism` enum (`hub_zmq_queue.hpp:175`) is ZMQ-shaped (`Uninitialized | Curve`); `RoleAPIBase::queue_mechanism(side)` dynamic_casts to `ZmqQueue*` and returns `Uninitialized` for SHM channels — last script-visible leak of transport-specific concepts at the queue-API surface.  Designer decision: Option A widen enum + move to `hub_queue.hpp` + virtualize `mechanism()` on `QueueReader` (~30 LOC + cross-engine binding); Option B drop `api.queue_mechanism` in favor of `is_channel_ready` (~50 LOC removal + caller migration).  Sibling leak: `QueueReader::is_shm_backed()` virtual at `hub_queue.hpp:346` — also a transport-discrimination escape hatch. |
|  | | ⏸ | **REVIEW-A — milestone A.**  Full systematic review (parallel agents: doc-vs-code + dead-code + gaps + edge-case) after #266+#267+#268+#269+#279 land.  Confirms the cleanup chain is consistent before consumer-dial starts.  |
...
|  | | ⏸ | **1i-coverage** (#270, file as part of 1j): L2 coverage for `RoleHostFrame::spawn_shm_auth_listener_` + producer/processor `prepare_tx_capability_` (no L2 covers role-host wiring today; orchestrator + AttachProtocol tested in isolation only). |
