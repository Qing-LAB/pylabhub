...
| **Status** | 🟢 **DESIGN FINAL; PHASE 1 IN FLIGHT** — substeps 1a-1h ✅; 1i-mig-1/2a/2b-1/2b-2/2c/M3/3 ✅ (TX side complete on both producer + processor); **pre-REVIEW-A cluster #266/#267/#268/#269/#279 ALL ✅** (M3.5 + doc-sync + producer hardening + consumer_attach scope + Mechanism enum widening — shipped 2026-06-22); **REVIEW-A ✅ closed 2026-06-22** (4 angles parallel review; 4 blockers → close-out commit; 1 HIGH BRC UAF finding tracked as #280 + blocks #272); 1i-mig-4 (consumer dial, #272) / 1i-mig-5 / 1i-cleanup (#275) / 1j (#257) / 1k (#258) / #262 mutual auth ALL ✅ shipped; REVIEW-C (#276) in progress; REVIEW-D/E remain.  Live tracker at §10.1. |
| **Created** | 2026-06-16 |
...
### 5.5 Attach-time L2 handshake — moved to HEP-CORE-0044

**Content moved 2026-07-08.**  The application-layer challenge-response
that runs between the connecting consumer and the producer over the Unix
socket is now specified in **HEP-CORE-0044 (AttachProtocol)** as a
transport-agnostic primitive.  HEP-0044 covers:

- Wire spec — Frame 1 (nonce + challenge), Frame 2 (proof + identity),
  Frame 3 (mutual-auth producer proof, opt-in) — see HEP-0044 §3.
- Cryptographic contract — SMS `box_encrypt_using` /
  `box_decrypt_using` name-based citation, nonce discipline,
  constant-time compare, plaintext scrub — see HEP-0044 §4.
- Per-connection state machine — see HEP-0044 §5.
- Transport-abstraction seam (`IAttachChannel`) and the SHM binding
  (`ShmAttachChannel`) — see HEP-0044 §6.
- Transport-agnostic protocol helpers (`run_producer_handshake` /
  `run_consumer_handshake`) — see HEP-0044 §7.
- Frame 3 mutual-auth extension (task #262) — see HEP-0044 §8.
- `role_type` extension used by broker-observer (HEP-CORE-0045) — see
  HEP-0044 §9.

**SHM-specific bookkeeping that stays in HEP-0041:**

- Per-connection SO_PEERCRED uid check runs BEFORE the AttachProtocol
  state machine.  It is the SHM binding's transport-layer identity
  check (HEP-CORE-0036 §I8); the crypto handshake in HEP-0044 §5's
  state machine begins at the ChannelBound state, after the fd is
  wrapped in `ShmAttachChannel`.
- The seckey representation contract (`SeckeyAccessor` receiving RAW
  32 BYTES) that HEP-CORE-0040 §8.5.2 established during substep 1c
  (#291 root cause) has been replaced by the SMS name-based citation
  in HEP-CORE-0043 §6 + HEP-CORE-0044 §4.1.  HEP-0040 §8.5.2 remains
  authoritative for the boundary contract; the AttachProtocol code no
  longer takes a `SeckeyAccessor` — it takes a KeyStore entry name
  string (`own_seckey_name`).
- Post-handshake, the producer completes the SHM-specific `SCM_RIGHTS`
  memfd handover — see §D4 below (steps 5-8).
...
### D4.5 Mutual auth — moved to HEP-CORE-0044

**Content moved 2026-07-08.**  The Frame 3 mutual-auth extension is now
specified in **HEP-CORE-0044 §8** as the shared opt-in mechanism for the
AttachProtocol primitive.  Coverage in HEP-0044 §8:

- Threat closed (same-uid producer impersonation).
...
**Rejected alternatives** (retained for reviewer context):

- **SO_PEERCRED + broker naming only** — weaker; single point of
  compromise at broker.  Rejected.
...
|  | | ✅ | **1i-mig-4 (#256/#272) — SHIPPED**: consumer dial in `RoleAPIBase::apply_consumer_reg_ack` — receive `shm_capability_endpoint` + `producer_pubkey_z85` from CONSUMER_REG_ACK; build `IShmCapabilityConsumer` via `attach_shm_capability_consumer`; run consumer half of §5.5 handshake; receive fd via SCM_RIGHTS; populate `RxQueueOptions::shm_capability_fd` (new field); add `build_rx_queue` capability-fd dispatch branch; decide fd-ownership site (designer call: RoleAPIBase Impl member vs ConsumerRoleHost member analogous to producer's `shm_transport_`).  ~150 LOC main change. |
|  | | ✅ | **1i-mig-5 (#256) — SHIPPED**: cutover + test fixups — L3 worker fixtures that set `cfg.shared_secret` migrate to capability path; L2 mutex tests retire; verify all reachable production callers use capability. |
|  | | ✅ | **REVIEW-B — milestone B — CLOSED (commit `8122bafc`).**  Full systematic review after 1i-mig-4+5 land.  Consumer dial is the largest single piece; this is the critical gate before legacy deletion. |
|  | | ✅ | **1i-cleanup (#256/#275 S1-S5) — SHIPPED**: delete the legacy machinery per the original §7 list + 2026-06-22 audit (`shm_config.hpp` dead reader + field, `hub_shm_queue.cpp` `set_shm_secret`/`apply_master_approval` SHM extractor/legacy `start()` branches/legacy factories, `data_block.cpp` plaintext stamp + memcmp + named-shm `find_datablock_*_impl`, `hub_state.hpp` `ChannelAccessEntry::shm_secret` field + `_on_channel_access_opened` 2nd param, `broker_service.cpp` hardcoded-zero call, L2 test fixtures pinning the dead field).  **MUST follow `docs/IMPLEMENTATION_GUIDANCE.md` § "Core Structure Change Protocol" for removing `SharedMemoryHeader::shared_secret[64]` — this triggers a layout-hash bump.**  Designer call on whether `data_block_recovery.cpp` keeps the field-shaped recovery roundtrip. |
|  | | 🟡 | **REVIEW-C — milestone C — IN PROGRESS 2026-07-16 (#276).**  Full systematic review after 1i-cleanup lands.  Confirms deletion is complete + nothing reachable from production + layout-hash bump consequences caught + recovery path still intact. |
...
## 10.5 Broker-side SHM metrics observability — moved to HEP-CORE-0045

**Content moved 2026-07-08.**  The broker-side observer pathway — how
the broker regains real-time SHM channel metrics under HEP-0041's
capability-transport model — is now the subject of **HEP-CORE-0045
(Broker SHM Channel Observer)**.

Coverage in HEP-0045:

- Motivation + threat model (§0-§1).
- Design decisions D1-D5 (trust anchor, producer stashing, dial timing,
  cross-platform teardown, opt-out) with tradeoff tables and verdicts
  (§2).
- Trust chain — broker keypair generation, `PRODUCER_REG_ACK` emission,
  producer-side storage, `AttachProtocolAcceptor` verify path (§3).
- Observer capability — `DataBlockObserverHandle` header-page-only view
  + `datablock_get_metrics_from_fd` C API (§4).
- Broker dial + fd cache worker (§5).
- Producer teardown + cross-platform `PeerDeathWatcher` (§6).
- Producer opt-out (§7).
- Broker `metrics_source` wiring (§8).
- Test coverage plan (§9).
- Slice map for task #317 (§10).

The observer pathway uses the `role_type="observer"` extension on
Frame 2 defined in **HEP-CORE-0044 §9** — the wire-level primitive is
shared with the consumer-attach path; the trust anchor is what
distinguishes them.
...
1. Phase 1 substeps 1i-mig → 1k are all shipped (1i-mig-4 #272, 1i-cleanup #275, 1j #257, 1k #258) and #262 mutual auth landed; the remaining Phase-1 work is the REVIEW-C/D/E production-readiness gates.
