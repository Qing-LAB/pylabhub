...
| 1i | #256 | ⏸ | **Split into 1i-mig (production wiring) + 1i-cleanup (delete legacy) per 2026-06-18 review.**  Original §7 1i description (delete `ChannelAccessEntry::shm_secret` + `set_shm_secret` API + memcmp checks + named-shm path + etc.) is correct in spirit but enumerates DELETIONS only — would break SHM channels unless the new path is wired first.  Migration sub-substeps (5, revised — earlier draft proposed a separate lifecycle module for the orchestrator accept thread; corrected after 2026-06-18 audit revealed it should live on the role host’s existing ThreadManager, same pattern as DataLoop + BRC poll + RxQueue poll): (1i-mig-1) `ShmQueue` fd plumbing (new `set_shm_capability_fd` setter + `start()` fd-source branch + Standby-mode reader factory); (1i-mig-2) producer role host wires `IShmCapabilityProducer` + `ShmAttachOrchestrator`, REGISTERS the orchestrator accept thread with its own existing `ThreadManager` (NOT a new lifecycle module — ZapPumpThread is a PROCESS-SINGLETON lifecycle module (one ZAP pump per process, shared across all roles); the SHM orchestrator is per-role-per-channel and belongs on the role host’s ThreadManager, same category as DataLoop / BRC poll / RxQueue poll), and populates `TxQueueOptions::shm_capability_fd`; (1i-mig-3) processor role host symmetric OUT-side wiring; (1i-mig-4) consumer dial in `RoleAPIBase::apply_consumer_reg_ack` (D2=a per 2026-06-18 review) + `RxQueueOptions::shm_capability_fd` populate; (1i-mig-5) cutover + test fixups.  Then 1i-cleanup deletes the legacy machinery per the original §7 list + the additional items found in the 2026-06-18 review (`shm_config.hpp:41` dead reader, `hub_shm_queue.cpp:379-389` dead extractor, etc.).  Open dependencies to verify in the next session: (a) is `PeerAllowlist` populated for SHM channels today (HEP §6.5 says yes for both ZMQ + SHM); (b) producer-side `BrokerRequestComm::consumer_attach` callback shape for the orchestrator's `BrokerQuery`; (c) `AttachProtocolAcceptor` seckey access via KeyStore lookup. |
