...
- **ConsumerSyncPolicy** – Added `ConsumerSyncPolicy` enum (Latest_only, Sequential, Sequential_sync) to config and header. Implemented: Latest_only = read commit_index only; Sequential = one shared read_index, consumer reads in order, writer blocks when (write_index - read_index) >= capacity; Sequential_sync = per-consumer positions in reserved_header, read_index = min(positions), writer backpressure. Tests: policy_latest_only, policy_single_reader, policy_sync_reader.
...
- **High-load integrity test** – high_load_single_reader worker: Sequential policy, ring capacity 4, scaled iterations (50k/5k); producer writes monotonic uint64_t, consumer asserts slot_id and payload in order. Test: HighLoadSingleReaderIntegrity.
