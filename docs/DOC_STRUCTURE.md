...
| HEP-CORE-0005 | Script Interface Abstraction Framework | **Archived** — superseded by HEP-CORE-0011 |
...
> **HEP-CORE-0047**, which maps every messaging HEP and every wire message.
> Refreshing this whole index is a tracked chore (`MESSAGEHUB_TODO.md`).
...
| Location | Purpose |
|----------|---------|
| **`examples/`** | Example code and usage documentation for the C++ stack (e.g. DataHub producer/consumer, schema, RAII layer). Not part of the default build. |

When you want to add or update **example code for usage** (standalone `.cpp` files or `.md` documentation with code snippets showing how to use the APIs), put it in **`examples/`**. This directory lives at the codebase level, not under `docs/`. 

**Guidelines:**
- Use `examples/` for runnable example code and API usage demonstrations
- Use `examples/README.md` to index all examples with brief descriptions
- Keep pure prose documentation in `docs/`; keep code-demonstrating examples in `examples/`
...
| Example code for usage (APIs, RAII, producer/consumer) | **`examples/`** |
