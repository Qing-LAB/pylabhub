...
| `<hub_uid>.vault` | 0600 (owner only) — **enforced at startup** | Encrypted: CurveZMQ private key + admin token.  Filename embeds the hub UID per HEP-CORE-0033 §6.5 (revised 2026-05-31). |
| Vault parent directory (e.g. `vault/`) | 0700 (owner only) — **enforced at startup** | Holds the `*.vault` file.  At write time `plh_hub --keygen` / `plh_role --keygen` set 0700 explicitly even when the operator's umask would permit looser. |
| `hub.pubkey` | 0644 | Broker CurveZMQ public key (share with roles) |
| `<role_uid>.vault` | 0600 (owner only) — **enforced at startup** | Encrypted: role CurveZMQ private key |
...
> **HEP-CORE-0035 §4.6 enforcement (added 2026-06-01).**  Both
> binaries verify the vault file mode = 0600 AND the parent
> directory mode = 0700 BEFORE reading the secret on every
> startup (including `--validate` is not affected; runmode and
> any path that calls `load_keypair` is).  On violation the
> binary refuses to start with an OpenSSH-style actionable
> diagnostic citing `HEP-CORE-0035 §4.6.2` and including the
> exact `chmod` command to fix.  The same modes are enforced
> *at write time* by `--keygen` (atomic `O_CREAT|O_EXCL|O_NOFOLLOW`
> + `fchmod 0600` for the vault file, `chmod 0700` for the
> parent dir), so a freshly-generated vault always satisfies
> the contract regardless of operator umask.

### 9.1.1 Upgrading from pre-2026-06-01 builds

Vaults written by pre-2026-06-01 builds **may have file mode ≠
0600 or parent directory mode ≠ 0700** (the mode depended on the
operator's umask at `--keygen` time).  After upgrading the binary,
running `plh_hub` / `plh_role` in runmode against an unmigrated
vault fails with:
...
```bash
chmod 0600 <hub-dir>/vault/<hub_uid>.vault
chmod 0700 <hub-dir>/vault

# Repeat for each role:
chmod 0600 <role-dir>/vault/<role_uid>.vault
chmod 0700 <role-dir>/vault
```

The vault password and contents are unchanged — only the file-mode
floor is now enforced at startup.  This was deliberately made a hard
break (rather than auto-heal at startup) because "security floor
auto-heals silently" is the antipattern HEP-CORE-0035 §4.6 was
written to prevent.  See HEP-CORE-0035 §4.6.2 for the full
