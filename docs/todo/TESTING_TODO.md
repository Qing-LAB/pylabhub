...
|---|---|---|---|
| 2026-08-10 | `test_role_vault.cpp:MoveConstructor_TransfersOwnership` and `test_hub_vault.cpp:MoveConstructor_TransfersOwnership` | "moving a vault carries its secret half across" | **NOT a retirement — the contract ceased to exist.** Under HEP-CORE-0035 §4.6.6 the vault holds no secret: `open`/`create` deposit into the KeyStore and return metadata only, so there is no secret for a move to carry and the old assertion would be vacuous. Both tests were REPLACED IN PLACE by `MoveConstructor_TransfersMetadata_*`, which pin the property that took over: the secrets' lifetime is the KEY STORE's, and neither moving nor DESTROYING the vault disturbs them. Strictly more coverage than before — the destroy case was never tested. |
| 2026-08-10 | `test_hub_vault.cpp:CreateReturnsValid64CharHexAdminToken` | "the admin token is a 64-char hex string" | **Contract changed, not retired.** The token is now 32 RAW bytes in locked memory (§4.6.6 secret section); hex is only the wire form an operator presents. Replaced in place by `CreateMintsA32ByteAdminTokenIntoLockedMemory`, which pins the raw length AND the verification path end-to-end — correct token accepted, one-character change rejected, truncation rejected, absent name rejected. Mutation-verified 2026-08-10 (forcing the comparison to succeed fails the test). |
...
| 2026-06-30 | `test_datahub_role_identity_policy.cpp:RoleIdentityPolicyEnumTest.*` (4 TEST_F's: ToStrAllValues, FromStrKnownValues, FromStrUnknownFallsToOpen, ToStrFromStrRoundTrip) | Pure value-conversion contract for `role_identity_policy_to_str` / `_from_str` (HEP-CORE-0035 §1 placeholder mechanism enum↔string helpers) | **RETIRED — record corrected 2026-08-07.**  This row used to claim "RE-LAYERED (not retired): moved verbatim to `tests/test_layer2_service/test_role_identity_policy.cpp`".  **That file does not exist, and `RoleIdentityPolicyEnumTest` appears nowhere under `tests/`** — the re-layer was planned but the enum was deleted first, so the tests went away with their subject.  The row's second claim is stale for the same reason: the helpers do *not* "stay in production for `broker_service.cpp::check_role_identity`" — that gate was deleted under #68 item 3 (redundant with the ZAP pubkey allowlist, HEP-0035 §4.5), and `role_identity_policy_to_str`/`_from_str` exist nowhere in the tree.  **No destination and none needed:** the contract these four pinned (enum↔string round-trip) died with the enum.  What replaced the *gate* is pinned by the ZAP/allowlist suites (`test_pubkey_origin.cpp`, `zap_router_workers.cpp`, `test_plh_hub_known_roles.cpp`).  Original source: REVIEW_AUTH6_TestDisposition_2026-06-27.md File 10 Suite 1. |
...
> **Extracted 2026-07-18 (✅ SHIPPED 2026-07-11):** Queue-owned topology + layer
> cleanup test coverage (P1-P3).  Lasting record: HEP-0011 §"Loop-ready gate" +
> HEP-0036 §I9.1.  Verbatim at commit `633d51c0`; index
...
| L3 ShmQueue coverage loss from the S3c retirement | ✅ **CLOSED — never was open** | All four contracts are pinned and passing in `tests/test_layer2_service/test_hub_shm_queue_contract.cpp` (`ShmQueueNoFlexzone`, `ShmQueueFlexzoneBidirectional`, `ShmQueueVerifyChecksumMismatch`, `DataBlockProducerRemapStubsThrow`), built by `test_layer2_hub_shm_queue_contract` at `CMakeLists.txt:576`. The earlier row called `CMakeLists.txt:566-569` a "tombstone listing what was lost" — **it is the header comment of the `add_executable` that builds the replacement.** The names were a manifest, read as an epitaph. Was #128; withdrawn 2026-08-07. |
...
- **Review audit (2026-07-16) — 3-agent systematic review of the sweep
  ...
  tests).  `datahub_slot_allocation_workers` — RESOLVED 2026-07-16 (belongs to
  #275/#276, not #52).  It had been masked 2026-06-30 as "coverage migrated,
  deletion #276", but the file WAS the fd-source destination ② the ShmQueue
  retirement pointed to — masking it orphaned real DataBlock-layout coverage
  (cache-line rounding, ring-capacity → total_block_size scaling, roundtrip)
  that lives nowhere else.  Wired in-place: un-masked from CMakeLists, added
  driver `test_datahub_slot_allocation.cpp` (`DatahubSlotAllocationTest`, 3
  tests), removed the Rule-6 doc-blocks, dropped the dead `secret` param.
  Design-validation against HEP-CORE-0002 caught a real test bug: the
  roundtrip worker never called `commit()`, so consumers timed out — fixed
  (acquire → fill → commit → release).  3/3 green.
