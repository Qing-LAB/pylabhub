...
| **C5** | CURVE-engagement test assertions + `Mechanism` invariant + start() guard | Added the `Mechanism` enum (`Uninitialized`/`Plaintext`/`Curve`) on `ZmqQueue` (HEP-CORE-0035 §2 enforcement point), a thread-safe atomic `mechanism_` field on `ZmqQueueImpl`, and a public const `ZmqQueue::mechanism()` accessor.  `start()` queries libzmq via `zmq_getsockopt(ZMQ_MECHANISM)` after CURVE setsockopts + bind/connect; if the answer is not `ZMQ_CURVE` the start fails (mechanism stays `Uninitialized`).  Four assertion classes:<br>• **L2 ZmqQueue** ⏳ Phase 2 in progress: `Mechanism_BeforeStart_IsUninitialized` + `Mechanism_AfterPushBind_IsCurve` shipped this round; `Mechanism::Plaintext` is structurally unreachable now that the guard is in place.<br>• **L2 RoleAPIBase** ⏳: `static_assert` that the ctor signature can't be constructed without the keystore-name args (covered by C3 — confirm + pin).<br>• **L3 broker** ⏳: spin up a CURVE-enforced producer; have a NULL-mech `zmq::socket_t` try to connect; assert handshake-failed event via socket monitor.<br>• **L2 loader contract + L4 gate** ⏳: one L2 unit test on the key-loading function (broken input → no valid key returned) + one L4 test that `plh_role` aborts before binding any data socket when the loader produces no valid key.  Both halves of the "illegal CURVE key → no connection allowed" gate. | shipped (#161, Phase 1 + 2 — `Mechanism` enum + start() guard + L2 invariant tests).  Remaining: L2 RoleAPIBase static_assert; L3 NULL-mech handshake-fail; L2 loader contract + L4 gate enforcement. |
