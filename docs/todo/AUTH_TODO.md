...
| 1i-coverage | #270 | ⏭ folded into #258 | **LAYER PIVOT 2026-06-25** — initial L2 implementation (`8716d91a` + `31b1d2af`) reverted same day after designer review.  The shim subclassing `RoleHostFrame` with a parallel `worker_main_` was a parallel production scaffold rather than a test of production.  The three methods are setup-step contributors to a multi-process auth flow; the natural test unit is multi-process, not unit.  Coverage folded into #258 L4 e2e where real `plh_hub` + `plh_role` binaries exercise these methods in their real production context.  Production INFO markers `event=ShmCapabilityTransportBound` + `event=ShmAcceptLoopSpawned` are observable from L4 captured stderr.  See HEP-0041 §10.1 1i-coverage row for full reasoning + lessons captured. |
