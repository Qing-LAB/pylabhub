...
|---|---|---|---|
| P-InboxQueue | InboxQueue CURVE = **pilot for the two-tier key model** | Now (post Phase-1-production-ready) | **RE-SCOPED 2026-07-17 (design decision).**  Inbox is the pilot for the **two-tier key model** (HEP-CORE-0036 Amendment 2026-07-17; HEP-CORE-0027 §3.5): a **per-process session keypair** for role↔role comm (fresh each start), separate from the persistent identity/record key used only for broker admission.  The broker distributes session pubkeys via the existing allowlist path (advertised inside the identity-authenticated REG).  Supersedes the earlier "reuse identity key + inherit single allowlist" MVP.  Plan: (1) pilot on inbox end-to-end; (2) THEN evaluate migrating the ZMQ data plane off §I6 identity keys.  Task **#191**.  Inbox state still follows parent ZmqQueue's Standby/Configured/Active machine. |
