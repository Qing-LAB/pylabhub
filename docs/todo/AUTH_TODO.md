...
- **ZMQ multi-producer fan-in ✅ shipped 2026-07-08** as `ZmqE2E_MultiProducer_TwoAuthorized` (Scenario C in the same file).  Pins the §7.1 loop under fan-in (`attach:begin producers=2` → `attach:success` per producer → `attach:complete admitted=2/2`) + both producers emitting `event=ChannelAuthApplied` + data flow through the queue.  L4 test surfaced a HEP-CORE-0017 §3.3 future-work gap: `ZmqQueue`'s PULL socket only connects to `peer[0].endpoint` even though `producer_peers_` holds all N — documented in `hub_zmq_queue.cpp:991` block comment and tracked in TESTING_TODO under the review-B follow-ups.
