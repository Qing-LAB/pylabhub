...
> **Overlaps #124 (2026-08-07).** The demo-config edits in this phase touch
> the same 27 files that ship `"keyfile": ""` and are unauthenticated since
> strict CURVE landed. Do them as one wave — and per #127, the target shape
> is demo configs generated by `--init` rather than hand-maintained, which
> makes this phase's checklist smaller, not bigger.
...
- [x] ~~HEP-CORE-0045 broker observer — verify unaffected.~~ **VOID
  2026-08-07** — HEP-CORE-0045 is ⛔ RETIRED, not deferred. There is no
  observer to be unaffected. Its residual work is #117 → #119 → removal of
  the shipped pieces.
