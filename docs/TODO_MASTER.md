...
| **M1.4** | Retire `metrics_store_` + `METRICS_REPORT_REQ`.  G1 (commit `416cbec`) fixed the most pressing piece (`ChannelMetrics::producer` overwrite bug) by making it per-uid.  M1.4 finishes the job: delete the legacy store entirely; route admin metrics queries through HubState's per-presence rows (HEP-CORE-0019 §2.3 Phase 6 already specifies this); delete `update_producer_metrics` / `update_consumer_metrics` / `query_metrics` / `handle_metrics_report_req`.  **Also closes H34** (`metrics_store_` per-uid leak surfaced in Wave M3 sixth-pass review): pre-existing leak where multi-producer DEREG / pending-timeout / consumer-liveness sweep leave per-uid entries in `metrics_store_[channel].{producers,consumers}[uid]` until channel teardown.  Naturally clean under H18 erase semantics once routing moves to HubState. | `docs/tech_draft/M1_FSM_consolidation_handoff_2026-05-09.md` §M1.4 | Wave M3 is now CLOSED (2026-05-11) — M1.4 unblocked.  Start when ready. | `docs/todo/MESSAGEHUB_TODO.md` |
