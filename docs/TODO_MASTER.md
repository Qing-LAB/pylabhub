...
| **Wave-B M4** | `RoleAPIBase` pImpl delegates via handler | ✅ shipped 2026-05-16 (M4a-f) | M4a-e: `src/utils/service/role_api_base.cpp` routes Class A through `pImpl->resolve_bc_for_channel(channel)`, Class B through `pImpl->resolve_bc_for_role()` (HEP-CORE-0033 §18.3 fall-through), Class D through `pImpl->resolve_bc_for_band(band)` with `handler->on_band_joined/on_band_left` wiring on band_join/band_leave success.  **M4f closed (commit `6da5837`)**: legacy `broker_channel` field + `set_broker_comm` + `start_ctrl_thread` deleted from `role_api_base.{hpp,cpp}`; no fallback view remains.  Commits: M4a `1d943b2`/`d20a8fc`, M4b `8c3994c`, M4c `5d927e0` + review follow-up `bff5c61`, M4d `3069d4d`, M4e `ddbb742`, M4f `6da5837`. |
...
**Arc B net status (REFRESHED 2026-05-19)**: Wave A + M0..M8 all shipped.  M4f closed in commit `6da5837` on 2026-05-16.  Every production role binary (producer / consumer / processor) runs on handler-mode end-to-end with no legacy `broker_channel` fallback view remaining.  **M9 (`RoleHostFrame<HostT>` template consolidation) is the only remaining Wave-B milestone — pure refactor, no new capability.  Wave-C (HEP cross-ref closure + archive transient drafts) and Wave-D (demo rewrite, blocked on Task #44 L4 test infrastructure) follow.**
