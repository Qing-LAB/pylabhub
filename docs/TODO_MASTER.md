...
| **#124** 27 demo configs ship `"keyfile": ""` | **3** | Broken since strict CURVE landed in May 2026.  Pairs with #127 — do it as one wave, with the configs as tool output rather than hand-maintained fixtures.  (The old "blocked by #123" link is gone; #123 was inverted.) |
...
- **Phase 3 — now #127** — CLI `--init` one-shot bundling (`--init` mode flag
  parses; bundling incomplete).  ~~Pulls in #123~~ — **that dependency was
  bogus and is removed**: `--init` gets the pubkey where the operator does,
  from `plh_role --keygen` stdout (HEP-0035 §4.8.4), so it has plenty to wire
  into `known_roles`.  Still pulls in
