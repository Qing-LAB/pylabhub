...
- [x] Phase 3 (2026-04-27, commits `92775ac` + `87390c8`): Wire protocol + broker dispatch — `REG_REQ` with `schema_packing` creates path-B records; `CONSUMER_REG_REQ` with `expected_packing` validates citation against the channel's record (path A); `SCHEMA_REQ` accepts `(owner, schema_id)` keying alongside legacy `channel_name`; inbox metadata creates `(role_uid, "inbox")` records; new NACK reasons (SCHEMA_HASH_MISMATCH_SELF, SCHEMA_CITATION_REJECTED, SCHEMA_UNKNOWN, SCHEMA_FORBIDDEN_OWNER, INBOX_SCHEMA_INVALID, INVALID_INBOX_PACKING). Broker now defensive against null SCHEMA_REQ payloads (DoS hardening). +13 Pattern-3 L3 tests in `test_datahub_broker.cpp`. Backward compat preserved (clients without new fields keep working). 1631/1631.
