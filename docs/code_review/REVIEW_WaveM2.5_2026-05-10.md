...
| F8 | `_set_channel_opened` insert_or_assign blocks step 3 mechanism | Critical | ✅ DESIGN LOCKED 2026-05-10 (step 3 mechanism chosen — see design doc §7.5: new `HubState::_on_producer_added` op with typed result struct; `_set_channel_opened` keeps original "open new" semantics; existing `_on_channel_registered` retained as test-only legacy primitive).  Implementation lands next. |
