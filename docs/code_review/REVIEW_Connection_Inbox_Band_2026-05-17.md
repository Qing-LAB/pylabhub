...
**Date:** 2026-05-17
**Verification pass 2026-08-07 (owner-directed): COMPLETE — 16 of 16 re-checked
against today's source.**  Every verdict below carries "verified 2026-08-07"
and the file:line that proves it.  Result: **8 resolved, 2 closed as accepted
design, 6 genuinely open.**

The 6 open findings are C5 (`Presence` has no `inbox_meta`), B4 (band always
binds `presences.front()`), X5 (pointer-stability contract enforced only by
comments), S4 (band membership has no local state), X2 (dead
`query_shm_info`), X4 (phase-label comments).  They are carried in the
subtopic TODOs; this review is closed and may be archived.
...
| C5 | D3 | Presence struct missing inbox_meta | ❌ OPEN — CONFIRMED (verified 2026-08-07) | `Presence` (`role_presence.hpp:222-267`) carries hub, channel, role_kind, slot_spec, fz_spec, connection, registration_state — **no `inbox_meta`**.  Inbox metadata still reaches the wire through `append_inbox_to_reg(reg, inbox_cfg_)` from the three role hosts (`consumer_role_host.cpp:343`, `processor_role_host.cpp:404,432`, `producer_role_host.cpp:387`), i.e. a per-host side-field, not a per-presence member.  **New in this pass:** the file's own header doc (`role_presence.hpp:6`) still describes the tuple as `(hub, channel, role_kind, schemas, inbox)` — the doc promises a field the struct does not have.  Fix the comment now even if the member stays deferred. |
...
| I3 | D2 | `found` field semantics | ✅ RESOLVED by different means (verified 2026-08-07) | The two divergent reply builders are gone: one `answer(found, reason, channel)` lambda (`broker_service.cpp:7023-7037`) emits a constant key set for every outcome, and `found` is now paired with an explicit `reason` — `no_such_role`, `no_inbox`, `not_reachable_yet`, `sender_not_registered`.  The ambiguity the finding named ("`found` reads as *role registered* but means *role has an inbox*") is answered by `reason` distinguishing those two cases outright, so the recommended `found`→`has_inbox` rename is no longer load-bearing.  Landed `6254b628` (2026-08-06); the role-side consumer of that vocabulary is `3b387c34`. |
...
| B4 | D2 | band_index_ always uses presences[0] | ❌ OPEN — CONFIRMED (verified 2026-08-07) | `RoleAPIBase::band_join` still calls `on_band_joined(channel, &presences.front())` (`role_api_base.cpp:5007`) — every band join binds presence 0 regardless of caller intent.  Neither remedy landed: `grep in_hub\|out_hub src/include/utils/role_api_base.hpp` returns nothing, so the `api.in_hub.band_join` / `api.out_hub.band_join` accessors (option a) do not exist, and the deferral is documented only in a code comment (`role_api_base.cpp:4977, 4996-5001`), not in HEP-0033 §18.3 (option b).  Harmless while every shipped role is single-presence; it becomes a correctness bug the day a dual-hub processor joins a band. |
...
| X5 | D2 | Presence::connection pointer stability contract | ❌ OPEN — CONFIRMED (verified 2026-08-07) | Still a `std::vector` + `connections_.reserve(presences_.size())` (`role_handler.cpp:56`) guarded by a CRITICAL comment (`:47-50`) and a header note (`role_handler.hpp:34-36`).  Neither recommended remedy landed — no debug-build mutation guard, no stable-storage container.  The invariant holds today because nothing mutates the vector after build; it is unenforced, so hub failover / dynamic rebinding would break `Presence::connection` (`role_presence.hpp:253`) silently. |
...
| S2 | D3 | RoleState 3-value + boolean latch | ✅ CLOSED — accepted design (verified 2026-08-07).  Code unchanged: `RoleState` is still `{Connected, Pending, Disconnected}` (`hub_state.hpp:67-77`) with `first_heartbeat_seen` (`:1131`) and the derived `ChannelObservable` mapping (`:108-110`).  The finding's own disposition was "acceptable, documented" with a conditional trigger — *if* the FSM ever gains a fourth state (e.g. Reconnecting), collapse the split representation.  That trigger has not fired, so this is a design note, not an open defect.  Nothing to carry. |
| S3 | D2 | start_handler_threads init-window mask state | ✅ FIXED 2026-05-19 (verified 2026-08-07) — exactly the recommended remedy.  `connection_alive_mask_` is stored in a new Phase 1.5 (`role_api_base.cpp:4353-4362`), **before** Phase 2 wires the lambdas that capture it and before Phase 3 spawns the ctrl threads (`:4447`).  The old Phase-4 site carries the pointer (`:4638`).  An `on_hub_dead` firing mid-startup now reads a mask that says "alive", so the transient wrong state is gone. |
| S4 | D2 | Band membership has no per-role local FSM | ❌ OPEN — CONFIRMED (verified 2026-08-07).  `RoleHandler::on_band_joined` (`role_handler.cpp:263-275`) is still a bare `band_index_[band_name] = presence` assignment, and `band_index_` is still `unordered_map<string, Presence *>` (`role_handler.hpp:349`).  No `BandRouting` record, no `Joining/Joined/Leaving` states anywhere in the tree.  A dropped BAND_JOIN REQ still leaves the role unable to answer "did I join?" without re-querying `band_members`. |
...
| TR2 | D2 | Role-side state-pinning needs first-class FSM | ✅ RESOLVED (verified 2026-08-07) — the S1 prerequisite landed, and the tests it unblocked exist.  `datahub_role_state_workers.cpp:1085-1142` pins the full transition sequence on a live presence: `Unregistered` pre-start → `Unregistered` still → `Registered` after REG_REQ → `Deregistered` after teardown.  `role_data_loop_workers.cpp:278` installs `Authorized` directly for the data-loop gate.  This is the transition-pinning the finding asked for, not outcome-only assertions. |
| TR3 | D3 | Mutation-sweep markers absent in older tests | ✅ CLOSED — won't-do, as recommended (verified 2026-08-07).  The finding's own remedy was "new tests should carry a mutation-sweep block; **existing tests are best left alone (churn cost too high)**".  The backfill half was never the ask, so there is no work item here.  The forward-looking half is a standing authoring rule and belongs in `docs/README/README_testing.md`, not in a review's open column. |
