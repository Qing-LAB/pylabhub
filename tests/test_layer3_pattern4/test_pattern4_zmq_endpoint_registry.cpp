...
class Pattern4ZmqEndpointRegistryTest : public pylabhub::tests::pattern4::Pattern4WireTest
{
  protected:
    ...
    {
        auto reply = c.request("REG_REQ", producer_reg_body(setup, channel, uid, /*shm=*/true),
                               "REG_ACK", ms{pylabhub::kLongTimeoutMs});
        ASSERT_TRUE(reply.has_value()) << "SHM producer REG_REQ timed out";
        ASSERT_EQ(reply->value("status", std::string{}), "success") << "body=" << reply->dump();
    }
    ...
    std::optional<nlohmann::json> endpoint_update(BrokerWireClient &c, const std::string &channel,
...
TEST_F(Pattern4ZmqEndpointRegistryTest, ZmqTransportRoundTrip)
{
    ...
    zmq::context_t ctx;
    auto prod = make_wire_client(ctx, setup, uid);
    ASSERT_NO_FATAL_FAILURE(register_producer_zmq_ep(prod, setup, channel, uid, zmq_ep));
    ASSERT_NO_FATAL_FAILURE(producer_heartbeat(prod, channel, uid));
...
TEST_F(Pattern4ZmqEndpointRegistryTest, ShmAndZmqCoexist)
{
    ...
    ASSERT_NO_FATAL_FAILURE(
        register_producer_zmq_ep(zmq_prod, setup, zmq_ch, zmq_uid, "tcp://127.0.0.1:55557"));

    auto list = shm_prod.request("CHANNEL_LIST_REQ", nlohmann::json::object(), "CHANNEL_LIST_ACK",
...
TEST_F(Pattern4ZmqEndpointRegistryTest, EndpointUpdateReflectedInDiscovery)
{
    ...
    ASSERT_EQ(upd->value("status", std::string{}), "success") << "body=" << upd->dump();
...
TEST_F(Pattern4ZmqEndpointRegistryTest, EndpointUpdateNonProducerReturnsError)
{
    ...
    ASSERT_NO_FATAL_FAILURE(
        register_producer_zmq_ep(prod, setup, channel, prod_uid, "tcp://127.0.0.1:0"));
...
TEST_F(Pattern4ZmqEndpointRegistryTest, ReqShapeNoUnmatchedRepliesForFireAndForget)
{
    ...
    nlohmann::json csum;
    csum["channel_name"] = channel;
    csum["role_uid"] = uid;
    csum["slot_index"] = 0;
    csum["error"] = "shape-probe";
    ...
    while (steady_clock::now() < deadline)
    {
        ...
        auto frame = prod.receive(duration_cast<milliseconds>(deadline - now));
        if (!frame)
            break;
        const std::string &mt = frame->first;
        const bool is_reply_shape =
            mt == "ERROR" || (mt.size() >= 4 && mt.compare(mt.size() - 4, 4, "_ACK") == 0);
        EXPECT_FALSE(is_reply_shape)
            << "fire-and-forget REQ produced a reply-shape message '" << mt
            << "' — HEP-CORE-0007 §12.2.1 violation; body=" << frame->second.dump();
    }
