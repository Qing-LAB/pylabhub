...
class Pattern4MetricsTest : public pylabhub::tests::pattern4::Pattern4WireTest
{
  protected:
    ...
    void heartbeat_with_metrics(BrokerWireClient &c, const std::string &channel,
...
TEST_F(Pattern4MetricsTest, HeartbeatMetricsStoredByBroker)
{
    ...
    expect_log(broker,
               "event=HeartbeatMetricsStored channel='" + channel + "' role_uid='" + uid +
                   "' role_type='producer'",
               milliseconds{pylabhub::kMidTimeoutMs});
    expect_log(broker, "\"iteration_count\":42", milliseconds{pylabhub::kMidTimeoutMs});
...
TEST_F(Pattern4MetricsTest, ConsumerHeartbeatMetricsStoredByBroker)
{
    ...
    heartbeat_with_metrics(cons, channel, cons_uid, "consumer", {{"read_count", 100}});

    expect_log(broker,
               "event=HeartbeatMetricsStored channel='" + channel + "' role_uid='" + cons_uid +
                   "' role_type='consumer'",
               milliseconds{pylabhub::kMidTimeoutMs});
    expect_log(broker, "\"read_count\":100", milliseconds{pylabhub::kMidTimeoutMs});
...
TEST_F(Pattern4MetricsTest, MetricsUpdateOverwriteOnHeartbeat)
{
    ...
    heartbeat_with_metrics(prod, channel, uid, "producer", {{"iteration_count", 10}});
    expect_log(broker, "\"iteration_count\":10", milliseconds{pylabhub::kMidTimeoutMs});

    heartbeat_with_metrics(prod, channel, uid, "producer", {{"iteration_count", 20}});
    // The second heartbeat stores the overwriting value.
    expect_log(broker, "\"iteration_count\":20", milliseconds{pylabhub::kMidTimeoutMs});
...
TEST_F(Pattern4MetricsTest, HeartbeatNoMetricsBackwardCompat)
{
    ...
    expect_log(broker, "first heartbeat received from role='" + uid + "'",
               milliseconds{pylabhub::kMidTimeoutMs});
