...
protected:
    ...
    {
        return c.request("CHANNEL_LIST_REQ", nlohmann::json::object(),
                         "CHANNEL_LIST_ACK",
                         std::chrono::milliseconds{pylabhub::kLongTimeoutMs});
    }
    ...
    static const nlohmann::json *find_channel(const nlohmann::json &list,
                                              const std::string    &name)
    {
        if (!list.contains("channels") || !list.at("channels").is_array())
            return nullptr;
        for (const auto &ch : list.at("channels"))
            if (ch.value("name", std::string{}) == name) return &ch;
        return nullptr;
    }
...
TEST_F(Pattern4BrokerAdminTest, ListChannels_FieldPresence)
{
    ...
    EXPECT_TRUE(entry->contains("producer_pid"))
        << "CHANNEL_LIST_ACK entry must carry producer_pid; entry="
        << entry->dump();
