...
std::unique_ptr<pylabhub::scripting::RoleHostBase> dummy_host_factory(pylabhub::config::RoleConfig,
                                                                      std::atomic<bool> *)
{
    return nullptr;
}

std::any dummy_config_parser(const nlohmann::json &, const pylabhub::config::RoleConfig &)
{
    return std::any{};
}
...
TEST(RoleRegistryTest, Get_AcceptsStringView_NoAllocationSemantics)
{
    ...
    RoleRegistry::register_runtime(tag).role_label("SV").host_factory(&dummy_host_factory).commit();
...
TEST(RoleRegistryTest, MultipleRoles_EachIndependentlyLookedUp)
{
    ...
    RoleRegistry::register_runtime(a).role_label("A").host_factory(&dummy_host_factory).commit();
    RoleRegistry::register_runtime(b).role_label("B").host_factory(&dummy_host_factory).commit();
    RoleRegistry::register_runtime(c).role_label("C").host_factory(&dummy_host_factory).commit();
