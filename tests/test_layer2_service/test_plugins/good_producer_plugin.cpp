...
extern "C" PLH_EXPORT bool on_produce(const plh_tx_t *tx)
{
    ...
    *slot = 42.0f;
    ...
    {
        g_ctx->report_metric(g_ctx, "produce_count", static_cast<double>(g_produce_count));
    }
    ...
    if (g_ctx)
    {
        ...
        if (g_ctx->band_members)
        {
            ...
            const int n = g_ctx->band_members(
                g_ctx, "!l2_test", +[](const plh_band_member_t *, void *) noexcept {}, nullptr);
    ...
    if (g_ctx)
    {
        g_test_inbox_ptrs_wired = (g_ctx->open_inbox && g_ctx->inbox_acquire && g_ctx->inbox_send &&
                                   g_ctx->inbox_discard && g_ctx->inbox_close)
                                      ? 1
                                      : 0;
        if (g_ctx->open_inbox)
        {
    ...
    if (g_ctx)
    {
        ...
        if (api.stop_reason() != plh::StopReason::Normal)
            g_test_cpp_wrapper_ok = 0;
        if (plh::to_string(plh::StopReason::CriticalError) != std::string_view{"critical_error"})
            g_test_cpp_wrapper_ok = 0;
        ...
        if (null_ctx.uid() != nullptr)
            g_test_cpp_wrapper_ok = 0;
        if (null_ctx.out_slots_written() != 0)
            g_test_cpp_wrapper_ok = 0;
        if (null_ctx.queue_mechanism() != plh::Mechanism::Uninitialized)
            g_test_cpp_wrapper_ok = 0;
        if (null_ctx.is_channel_ready("anything"))
            g_test_cpp_wrapper_ok = 0;
        if (null_ctx.is_critical_error())
            g_test_cpp_wrapper_ok = 0;
    ...
    if (g_ctx && g_ctx->report_metric)
    {
        ...
        g_ctx->report_metric(g_ctx, "test_band_send_ok", static_cast<double>(g_test_band_send_ok));
...
extern "C" PLH_EXPORT int test_get_init_count(void)
{
    return g_init_count;
}
extern "C" PLH_EXPORT int test_get_produce_count(void)
{
    return g_produce_count;
}
extern "C" PLH_EXPORT int test_get_stop_count(void)
{
    return g_stop_count;
}

// Query symbols for test verification.
extern "C" PLH_EXPORT uint64_t test_get_out_slots_written(void)
{
    return g_test_out_slots_written;
}
extern "C" PLH_EXPORT uint64_t test_get_in_slots_received(void)
{
    return g_test_in_slots_received;
}
extern "C" PLH_EXPORT uint64_t test_get_out_drop_count(void)
{
    return g_test_out_drop_count;
}
extern "C" PLH_EXPORT uint64_t test_get_script_error_count(void)
{
    return g_test_script_error_count;
}
extern "C" PLH_EXPORT size_t test_get_slot_logical_size(void)
{
    return g_test_slot_logical_size;
}
extern "C" PLH_EXPORT uint32_t test_get_spinlock_count(void)
{
    return g_test_spinlock_count;
}
extern "C" PLH_EXPORT int test_get_cpp_wrapper_ok(void)
{
    return g_test_cpp_wrapper_ok;
}

// Band API query symbols (set during on_produce).
extern "C" PLH_EXPORT int test_get_band_join_failed(void)
{
    return g_test_band_join_failed;
}
extern "C" PLH_EXPORT int test_get_band_leave_failed(void)
{
    return g_test_band_leave_failed;
}
extern "C" PLH_EXPORT int test_get_band_send_ok(void)
{
    return g_test_band_send_ok;
}
