...
{
    auto w =
        SpawnWorker("native_engine.full_lifecycle_produce_commits_and_writes_slot", {plugin_dir()});
    ExpectWorkerOk(w);
}

TEST_F(NativeEngineTest, HasCallback_ReflectsPluginSymbols)
{
    auto w = SpawnWorker("native_engine.has_callback_reflects_plugin_symbols", {plugin_dir()});
    ExpectWorkerOk(w);
}
...
{
    auto w =
        SpawnWorker("native_engine.schema_validation_matching_schema_succeeds", {plugin_dir()});
    ExpectWorkerOk(w);
}

TEST_F(NativeEngineTest, SchemaValidation_HasSchemaFalse_ReturnsFalse)
{
    auto w = SpawnWorker("native_engine.schema_validation_has_schema_false_returns_false",
                         {plugin_dir()});
...
{
    auto w = SpawnWorker("native_engine.schema_validation_mismatched_schema_fails", {plugin_dir()});
...
{
    auto w = SpawnWorker("native_engine.load_script_missing_required_callback_returns_false",
                         {plugin_dir()});
...
{
    auto w =
        SpawnWorker("native_engine.generic_invoke_known_callback_returns_true", {plugin_dir()});
    ExpectWorkerOk(w);
}

TEST_F(NativeEngineTest, SupportsMultiState_DefaultFalse)
{
    auto w = SpawnWorker("native_engine.supports_multi_state_default_false", {plugin_dir()});
    ExpectWorkerOk(w);
}

TEST_F(NativeEngineTest, ContextFieldsPassedToPlugin)
{
    auto w = SpawnWorker("native_engine.context_fields_passed_to_plugin", {plugin_dir()});
    ExpectWorkerOk(w);
}

TEST_F(NativeEngineTest, Checksum_WrongHash_RejectsPlugin)
{
    auto w = SpawnWorker("native_engine.checksum_wrong_hash_rejects_plugin", {plugin_dir()});
...
{
    auto w = SpawnWorker("native_engine.checksum_empty_hash_skips_verification", {plugin_dir()});
    ExpectWorkerOk(w);
}

TEST_F(NativeEngineTest, Api_CountersAndSchemaSize_ThroughNativeModule)
{
    auto w = SpawnWorker("native_engine.api_counters_and_schema_size_through_native_module",
                         {plugin_dir()});
    ExpectWorkerOk(w);
}
...
{
    auto w = SpawnWorker("native_engine.full_startup_producer_slot_only", {plugin_dir()});
    ExpectWorkerOk(w);
}
...
{
    auto w = SpawnWorker("native_engine.full_startup_producer_multifield", {plugin_dir()});
    ExpectWorkerOk(w);
}

TEST_F(NativeEngineTest, FullStartup_Consumer_Multifield)
{
    auto w = SpawnWorker("native_engine.full_startup_consumer_multifield", {plugin_dir()});
    ExpectWorkerOk(w);
}

TEST_F(NativeEngineTest, FullStartup_Processor_Multifield)
{
    auto w = SpawnWorker("native_engine.full_startup_processor_multifield", {plugin_dir()});
    ExpectWorkerOk(w);
}

TEST_F(NativeEngineTest, FullStartup_Producer_SlotAndFlexzone)
{
    auto w = SpawnWorker("native_engine.full_startup_producer_slot_and_flexzone", {plugin_dir()});
    ExpectWorkerOk(w);
}

TEST_F(NativeEngineTest, InvokeOnInbox_TypedData)
{
    auto w = SpawnWorker("native_engine.invoke_on_inbox_typed_data", {plugin_dir()});
    ExpectWorkerOk(w);
}
...
{
    auto w =
        SpawnWorker("native_engine.api_band_pub_sub_no_broker_graceful_return", {plugin_dir()});
    ExpectWorkerOk(w);
}
