...
{
    auto api = std::make_unique<RoleAPIBase>(core, "prod", "prod.testnative.uid00000001");
...
template <typename Fn> int run_ne_worker(Fn body, const char *name)
{
...
int full_lifecycle_produce_commits_and_writes_slot(const std::string &plugin_dir)
{
    return run_ne_worker(
        [&]()
        {
            ...
            ASSERT_TRUE(
                engine.load_script(plugin.parent_path(), plugin.filename().string(), "on_produce"));
...
int load_script_missing_file_returns_false()
{
    return run_ne_worker(
        [&]()
        {
            ...
            ASSERT_TRUE(engine.initialize("test", &core));
            EXPECT_FALSE(
                engine.load_script("/nonexistent/path", "libno_such_plugin.so", "on_produce"));
        },
...
int load_script_missing_required_callback_returns_false(const std::string &plugin_dir)
{
    return run_ne_worker(
        [&]()
        {
            ...
            ASSERT_TRUE(fs::exists(plugin));
            EXPECT_FALSE(
                engine.load_script(plugin.parent_path(), plugin.filename().string(), "on_tick"));
        },
...
int full_startup_consumer(const std::string &plugin_dir)
{
    return run_ne_worker(
        [&]()
        {
            ...
            auto api = std::make_unique<RoleAPIBase>(core, "cons", "cons.testnative.uid00000001");
...
int full_startup_processor(const std::string &plugin_dir)
{
    return run_ne_worker(
        [&]()
        {
            ...
            auto api = std::make_unique<RoleAPIBase>(core, "proc", "proc.testnative.uid00000001");
...
int full_startup_processor_multifield(const std::string &plugin_dir)
{
    return run_ne_worker(
        [&]()
        {
            ...
            in_slot.ts = 1.23456789;
            in_slot.flag = 0xAB;
            in_slot.count = -42;
            in_slot.values[0] = 1.0f;
            in_slot.values[1] = 2.5f;
            in_slot.values[2] = -3.75f;
    ...
    return run_ne_worker(
        [&]()
        {
            ...
            // file header BYPASS PATTERN).
            {
                ...
                fz_info.tx_logical_size = pylabhub::hub::compute_schema_size(spec, spec.packing);
...
int invoke_on_inbox_typed_data(const std::string &plugin_dir)
{
    return run_ne_worker(
        [&]()
        {
            ...
            engine.invoke_on_inbox({&inbox_data, sizeof(inbox_data), "prod.sender.uid00000001", 1});
            EXPECT_EQ(engine.script_error_count(), 0u);
    ...
    NativeEngineWorkerRegistrar()
    {
        register_worker_dispatcher(
            [](int argc, char **argv) -> int
            {
                ...
                auto dot = mode.find('.');
                if (dot == std::string_view::npos || mode.substr(0, dot) != "native_engine")
                    return -1;
                ...
                if (argc <= 2)
                {
                    fmt::print(stderr, "native_engine.{}: missing plugin_dir arg\n", sc);
                    return 1;
                }
