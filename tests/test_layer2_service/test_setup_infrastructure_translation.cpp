  ...
  protected:
    fs::path unique_dir(const char *prefix)
    {
        ...
        fs::path p = fs::temp_directory_path() /
                     ("plh_l2_translate_" + std::string(prefix) + "_" + std::to_string(::getpid()) +
    ...
    RoleConfig generate_and_load(const std::string &role_type, const std::string &cfg_filename,
                                 const std::string &name, Mutate &&mutate)
    {
        ...
        EXPECT_TRUE(fs::exists(cfg_path)) << "init_directory did not write " << cfg_path;
        ...
        EXPECT_NE(info, nullptr) << "RoleRegistry::get_runtime('" << role_type
                                 << "') returned null";
        return RoleConfig::load_from_directory(dir.string(), info->role_type.c_str(),
                                               info->config_parser);
    }
...
TEST_F(SetupInfrastructureTranslationTest, Producer_ShmTransport_AllFieldsCopied)
{
    ...
    const auto cfg = generate_and_load("producer", "producer.json", "TestProd",
                                       [](nlohmann::json &j)
                                       {
                                           ...
                                           // HEP-CORE-0041 substep 1h (#255) — out_shm_secret
                                           // retired (parser throws on this field); a separate
                                           // rejection test below pins the throw shape.  Removed
                                           // from this round-trip test; the legacy non-zero
                                           // assertion at the matching EXPECT_EQ below is also
                                           // dropped.
                                           j["out_shm_sync_policy"] =
                                               "sequential_sync";  // template = "sequential"
    ...
    const auto opts =
        pylabhub::producer::ProducerRoleHost::make_tx_opts(cfg, slot_spec, fz_spec, has_tx_fz);
    ...
    EXPECT_EQ(opts.shm_config.policy, pylabhub::hub::DataBlockPolicy::RingBuffer);
...
{
    const auto cfg = generate_and_load("producer", "producer.json", "TestProdZmq",
                                       [](nlohmann::json &j)
                                       {
    ...
    const auto opts = pylabhub::producer::ProducerRoleHost::make_tx_opts(cfg, slot_spec, fz_spec,
                                                                         /*has_tx_fz=*/true);
    ...
    EXPECT_FALSE(opts.has_shm) << "ZMQ transport must override has_shm to false";
    ...
    EXPECT_EQ(opts.zmq_overflow_policy, pylabhub::hub::OverflowPolicy::Block);
...
TEST_F(SetupInfrastructureTranslationTest, Consumer_ShmTransport_AllFieldsCopied)
{
    ...
    const auto cfg = generate_and_load("consumer", "consumer.json", "TestCons",
                                       [&ch](nlohmann::json &j)
                                       {
                                           j["in_channel"] = ch;
                                           j["in_transport"] = "shm";
    ...
    const auto opts = pylabhub::consumer::ConsumerRoleHost::make_rx_opts(cfg, slot_spec, fz_spec,
                                                                         /*has_rx_fz=*/true);
...
{
    const auto cfg = generate_and_load("consumer", "consumer.json", "TestConsZmq",
                                       [](nlohmann::json &j)
                                       {
                                           ...
                                           // that `build_rx_queue` forwards to
                                           // `hub::Queue::create_reader`).
...
TEST_F(SetupInfrastructureTranslationTest, Processor_ShmTransport_AllFieldsCopied)
{
    ...
    const auto cfg = generate_and_load("processor", "processor.json", "TestProc",
                                       [&](nlohmann::json &j)
                                       {
                                           j["in_channel"] = in_ch;
                                           ...
                                           j["out_shm_sync_policy"] =
                                               "latest_only"; // template = "sequential"
                                           j["checksum"] = "enforced";
                                           j["flexzone_checksum"] = true;
                                       });
    ...
    // ── Rx side ──
    const auto rx = pylabhub::processor::ProcessorRoleHost::make_rx_opts(cfg, slot_spec, fz_spec,
                                                                         /*has_rx_fz=*/true);
    EXPECT_EQ(rx.shm_name, in_ch) << "Processor.rx — B5 regression: shm_name must equal in_channel";
    // HEP-CORE-0041 1h (#255) — shm_shared_secret no longer settable
    ...
    // ── Tx side ──
    const auto tx = pylabhub::processor::ProcessorRoleHost::make_tx_opts(cfg, slot_spec, fz_spec,
                                                                         /*has_tx_fz=*/true);
    ...
    EXPECT_EQ(tx.shm_config.policy, pylabhub::hub::DataBlockPolicy::RingBuffer);
    EXPECT_EQ(tx.shm_config.consumer_sync_policy, pylabhub::hub::ConsumerSyncPolicy::Latest_only);
...
{
    const auto cfg = generate_and_load("processor", "processor.json", "TestProcZmq",
                                       [](nlohmann::json &j)
                                       {
                                           j["in_channel"] = "test.proc.zmq.in";
                                           j["out_channel"] = "test.proc.zmq.out";
                                           // Two topology cells in the same processor config
                                           // exercise the translator on both directions.  In =
                                           // fan-in (processor's input side is the binding side for
                                           // that channel); out = fan-out (processor's output side
                                           // is binding).  HEP-CORE-0017 §3.3.0 matrix rows
                                           // exercised: reader/fan-in ZMQ + writer/fan-out ZMQ.
                                           j["in_channel_topology"] = "fan-in";
                                           j["out_channel_topology"] = "fan-out";
                                           j["in_transport"] = "zmq";
    ...
    EXPECT_EQ(tx.zmq_overflow_policy, pylabhub::hub::OverflowPolicy::Drop);
    EXPECT_EQ(tx.checksum_policy, cfg.checksum().policy);
       ...
       Producer_SchemaSpec_Propagates_And_HasFzFalse_ClearsFlexzoneChecksum)
{
    ...
    const auto cfg = generate_and_load("producer", "producer.json", "TestProdQ23",
                                       [](nlohmann::json &j)
                                       {
    ...
    // (a) has_tx_fz=true — flexzone_checksum should follow config (true).
    {
        ...
        ASSERT_EQ(opts.fz_spec.fields.size(), 2u) << "fz_spec.fields must propagate non-empty (Q2)";
...
{
    const auto cfg = generate_and_load("consumer", "consumer.json", "TestConsQ23",
                                       [](nlohmann::json &j)
                                       {
...
TEST_F(SetupInfrastructureTranslationTest, Processor_SchemaSpec_Propagates_And_HasFzFalse_BothSides)
{
    const auto cfg = generate_and_load("processor", "processor.json", "TestProcQ23",
                                       [](nlohmann::json &j)
                                       {
...
TEST_F(SetupInfrastructureTranslationTest, RetiredShmSecretKeysRejected)
{
    EXPECT_THROW(
        {
            (void)generate_and_load("producer", "producer.json", "TestProdRetired",
                                    [](nlohmann::json &j)
                                    {
    ...
    EXPECT_THROW(
        {
            (void)generate_and_load("consumer", "consumer.json", "TestConsRetired",
                                    [](nlohmann::json &j)
                                    {
    ...
    try
    {
        (void)generate_and_load("producer", "producer.json", "TestProdRetiredMsg",
                                [](nlohmann::json &j)
                                {
