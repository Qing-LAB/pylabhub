 ...
 * @brief L2 protocol-validation tests for `seed_curve_identities()`
 *        (HEP-CORE-0040 §172 + HEP-CORE-0035 §4.6.5 + task #177).
 *
 * The seeder lives in `tests/test_framework/curve_test_setup.h` and is
 ...
 * identity).  It IS the protocol — the bypass-discipline sanctioned by
 * HEP-CORE-0035 §4.6.5 lives entirely in this seeding helper (skip
 * on-disk vault + Argon2id; keep real CURVE on the wire).
 *
 * These tests pin the seeder's contract directly so future refactors
 ...
 * `SecureSubsystem` (and its KeyStore member) is a process singleton
 * per HEP-CORE-0043 §1.3.
 *
 ...
 *     and L2 here proves seeder → KeyStore matches what vault would
...
class CurveIdentitySeederTest : public IsolatedProcessTest
{
};

// ─── §1: canonical-name pins (mutation-protect literal names) ────────────────

TEST_F(CurveIdentitySeederTest, Fixture_SeedsHubIdentity_UnderCanonicalName)
{
...
TEST_F(CurveIdentitySeederTest, Fixture_SeedsRoleIdentities_UnderRoleKeystoreName)
{
...
TEST_F(CurveIdentitySeederTest, Fixture_SeededBytes_MatchSetupForHubIdentity)
{
...
TEST_F(CurveIdentitySeederTest, Fixture_SeededBytes_MatchSetupForRoleIdentity)
{
...
TEST_F(CurveIdentitySeederTest, Fixture_SeedsExactlyTheDeclaredEntries)
{
