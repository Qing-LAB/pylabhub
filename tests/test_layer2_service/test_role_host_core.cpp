...
TEST_F(RoleHostCoreTest, FzSpec_SetAndRead)
{
    EXPECT_FALSE(core_.has_fz());
    EXPECT_EQ(core_.schema_fz_size(), 0u);
    ...
    core_.set_fz_spec(std::move(spec), 8192);

    EXPECT_TRUE(core_.has_fz());
    EXPECT_EQ(core_.schema_fz_size(), 8192u);
    EXPECT_TRUE(core_.fz_spec().has_schema);
    EXPECT_EQ(core_.fz_spec().packing, "aligned");
}
...
TEST_F(RoleHostCoreTest, FzSpec_NoSchema_HasFzFalse)
{
    ...
    core_.set_fz_spec(std::move(spec), 0);
