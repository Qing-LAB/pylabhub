...
{
    auto w = SpawnWorker("key_store.sodium_smoke", {unique_dir("sodium_smoke")});
    ExpectWorkerOk(w);
}
...
{
    auto w = SpawnWorker("key_store.add_identity_then_pubkey_and_with_seckey_roundtrip",
...
{
    auto w = SpawnWorker("key_store.add_identity_zeros_source", {unique_dir("zeros_source")});
    ExpectWorkerOk(w);
}
...
{
    auto w = SpawnWorker("key_store.pubkey_on_missing_throws", {unique_dir("pubkey_missing")});
    ExpectWorkerOk(w);
}

TEST_F(KeyStoreTest, WithSeckey_OnMissing_Throws)
{
    auto w = SpawnWorker("key_store.with_seckey_on_missing_throws", {unique_dir("seckey_missing")});
    ExpectWorkerOk(w);
}
...
{
    auto w = SpawnWorker("key_store.pubkey_on_raw_entry_throws", {unique_dir("pubkey_raw_type")});
    ExpectWorkerOk(w);
}
...
{
    auto w =
        SpawnWorker("key_store.with_seckey_on_raw_entry_throws", {unique_dir("seckey_raw_type")});
    ExpectWorkerOk(w);
}

TEST_F(KeyStoreTest, AddIdentity_Duplicate_Throws)
{
    auto w = SpawnWorker("key_store.add_identity_duplicate_throws", {unique_dir("duplicate")});
    ExpectWorkerOk(w);
}

TEST_F(KeyStoreTest, AddIdentity_WrongSize_Throws)
{
    auto w = SpawnWorker("key_store.add_identity_wrong_size_throws", {unique_dir("wrong_size")});
    ExpectWorkerOk(w);
}
...
{
    auto w = SpawnWorker("key_store.remove_makes_subsequent_lookup_throw", {unique_dir("remove")});
    ExpectWorkerOk(w);
}

TEST_F(KeyStoreTest, HasAndSize_TrackEntries)
{
    auto w = SpawnWorker("key_store.has_and_size_track_entries", {unique_dir("has_size")});
    ExpectWorkerOk(w);
}
...
{
    auto w = SpawnWorker("key_store.parallel_reads_do_not_block", {unique_dir("parallel_reads")});
    ExpectWorkerOk(w);
}

TEST_F(KeyStoreTest, Remove_BlocksBehindInFlightWithSeckey)
{
    auto w = SpawnWorker("key_store.remove_blocks_behind_in_flight_with_seckey",
...
{
    auto w = SpawnWorker("key_store.secure_buffer_dtor_zeroes", {unique_dir("secure_buffer")});
    ExpectWorkerOk(w);
}
...
{
    auto w =
        SpawnWorker("key_store.sms_instance_returns_same_reference", {unique_dir("sms_instance")});
    ExpectWorkerOk(w);
}
...
{
    auto w = SpawnWorker("key_store.sms_parallel_instance_calls", {unique_dir("sms_parallel")});
    ExpectWorkerOk(w);
}
...
{
    auto w = SpawnWorker("key_store.sms_wrappers_depth", {unique_dir("sms_wrappers_depth")});
    ExpectWorkerOk(w);
}
...
{
    auto w = SpawnWorker("key_store.with_seckey_z85_view_shape", {unique_dir("seckey_z85_shape")});
    ExpectWorkerOk(w);
}
...
{
    auto w =
        SpawnWorker("key_store.with_keypair_z85_yields_both_halves", {unique_dir("keypair_z85")});
    ExpectWorkerOk(w);
}
...
{
    auto w = SpawnWorker("key_store.box_encrypt_decrypt_roundtrip", {unique_dir("box_roundtrip")});
    ExpectWorkerOk(w);
}
...
void expect_sigabrt(pylabhub::tests::helper::WorkerProcess &w, const char *accessor)
{
    w.wait_for_exit();
    EXPECT_EQ(w.exit_code(), kSigabrtExitCode)
        << "Expected " << accessor << " to panic (SIGABRT = exit " << kSigabrtExitCode
        << "), got exit " << w.exit_code();
...
{
    auto w =
        SpawnWorker("key_store.panic_when_keys_called_without_sms", {unique_dir("panic_keys")});
...
{
    auto w = SpawnWorker("key_store.pwhash_argon2id_roundtrip", {unique_dir("pwhash_argon2id")});
    ExpectWorkerOk(w, {},
                   {
...
{
    auto w =
        SpawnWorker("key_store.derive_pwhash_salt_deterministic", {unique_dir("pwhash_salt_det")});
    ExpectWorkerOk(w, {},
                   {
...
{
    auto w = SpawnWorker("key_store.bin2hex_roundtrip", {unique_dir("bin2hex")});
    ExpectWorkerOk(w, {},
                   {
...
{
    auto w = SpawnWorker("key_store.verify_blake2b_roundtrip", {unique_dir("verify_blake2b")});
    ExpectWorkerOk(w, {},
                   {
...
{
    auto w = SpawnWorker("key_store.sodium_ready_agrees_with_lifecycle_initialized",
