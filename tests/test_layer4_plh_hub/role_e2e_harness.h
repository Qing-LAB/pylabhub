...
inline bool wait_for_hub_marker(const fs::path &dir,
...
inline bool wait_for_role_marker(const fs::path &role_dir,
                                 WorkerProcess &p,
                                 const std::string &marker,
                                 std::chrono::milliseconds timeout =
                                     std::chrono::milliseconds(10000))
...
inline std::string keygen_role_and_read_pubkey(
    const fs::path    &role_dir,
    const std::string &role_kind,
...
inline void add_known_role(const fs::path    &hub_dir,
