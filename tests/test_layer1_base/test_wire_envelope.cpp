...
TEST(WireEnvelope, EnvelopeHashDiffersOnAnyInputChange)
{
    ...
    EXPECT_NE(base, WireEnvelope::compute_envelope_hash("id-2", "REG_REQ", "cid-42"));
    EXPECT_NE(base, WireEnvelope::compute_envelope_hash("id-1", "REG_ACK", "cid-42"));
    EXPECT_NE(base, WireEnvelope::compute_envelope_hash("id-1", "REG_REQ", "cid-43"));
}
...
TEST(WireEnvelope, RouterRoundTripRecoversAllSkeletonFields)
{
    ...
    EXPECT_EQ(env->body().at("channel_name").get<std::string>(), "lab.test.channel");
}
...
TEST(WireEnvelope, DealerRoundTripUsesDealersOwnRoutingId)
{
    ...
    auto frames = WireEnvelope::build_dealer_send(dealer_own_routing_id, /*msg_type=*/"REG_ACK",
                                                  /*correlation_id=*/"cid-1", body);
    ASSERT_EQ(frames.size(), 4U);

    ParseError err{};
    auto env = WireEnvelope::parse_dealer_recv(std::move(frames), dealer_own_routing_id, &err);
    ASSERT_TRUE(env.has_value()) << "parse failed: err=" << static_cast<int>(err);
...
{
    auto body = make_reg_req_body();
    EXPECT_THROW(WireEnvelope::build_dealer_send("prod.test.uid1", "REG_REQ", "", body),
...
TEST(WireEnvelope, TamperingIdentityFailsHashCheck)
{
    ...
    auto body = make_reg_req_body();
    auto frames = WireEnvelope::build_router_send("identity-A", "REG_REQ", "cid-1", body);
    ASSERT_EQ(frames.size(), 5U);
...
TEST(WireEnvelope, TamperingBodyFailsHashCheck)
{
    ...
    auto frames_a = WireEnvelope::build_router_send("identity-A", "REG_REQ", "cid-1", body_a);
    ...
    auto frames_b = WireEnvelope::build_router_send("identity-A", "REG_ACK", "cid-1", body_b);
...
TEST(WireBodies, RegReqBodyValidatesRequiredFields)
{
    ...
    body_json["envelope_hash"] = WireEnvelope::compute_envelope_hash("id-x", "REG_REQ", "cid-1");
...
TEST(WireBodies, RegAckBodyRejectsMissingStatus)
{
    ...
    EXPECT_THROW(pylabhub::wire::RegAckBody{std::move(body)}, WireBodyError);
}
...
TEST(WireBodies, HeartbeatNotifyBodyRejectsMissingChannelName)
{
    ...
    body["role_type"] = "producer";
    body["envelope_hash"] = "deadbeef";
    EXPECT_THROW(pylabhub::wire::HeartbeatNotifyBody{std::move(body)}, WireBodyError);
}
...
TEST(WireBodies, EndpointUpdateReqBodyRejectsMissingEndpoint)
{
    ...
    EXPECT_THROW(pylabhub::wire::EndpointUpdateReqBody{std::move(body)}, WireBodyError);
}
...
TEST(WireBodies, DeregReqBodyRejectsMissingSecurityTriple)
{
    ...
    EXPECT_THROW(pylabhub::wire::DeregReqBody{std::move(body)}, WireBodyError);
}
...
TEST(WireBodies, AdminHelloReqBodyRejectsMissingToken)
{
    ...
    body["label"] = "alice-laptop";
    body["envelope_hash"] = "deadbeef";
    EXPECT_THROW(pylabhub::wire::AdminHelloReqBody{std::move(body)}, WireBodyError);
}
...
TEST(WireBodies, AdminCloseChannelReqBodyRejectsMissingChannel)
{
    ...
    body["session_id"] = "abcdef0123";
    body["envelope_hash"] = "deadbeef";
    EXPECT_THROW(pylabhub::wire::AdminCloseChannelReqBody{std::move(body)}, WireBodyError);
}
