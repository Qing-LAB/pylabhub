...
struct HubStateTestAccess
{
    ...
    static void set_shm_block(HubState &s, ShmBlockRef r) { s._set_shm_block(std::move(r)); }
    ...
    on_producer_added(HubState &s, const std::string &channel_name, ChannelSchemaInvariants schema,
                      ChannelTransportInvariants transport, ProducerEntry producer,
                      std::optional<ChannelTopology> declared_topology = std::nullopt)
    {
        return s._on_producer_added(channel_name, std::move(schema), std::move(transport),
                                    declared_topology, std::move(producer));
    }
    ...
    static bool set_producer_zmq_node_endpoint(HubState &s, const std::string &channel_name,
                                               const std::string &role_uid, std::string endpoint)
    {
        return s._set_producer_zmq_node_endpoint(channel_name, role_uid, std::move(endpoint));
    }
    static void on_channel_closed(HubState &s, const std::string &n, ChannelCloseReason why)
    {
    ...
    static void on_heartbeat_timeout(HubState &s, const std::string &ch, const std::string &uid)
    {
    ...
    static void on_band_joined(HubState &s, const std::string &band, BandMember m)
    {
        s._on_band_joined(band, std::move(m));
    }
    static void on_band_left(HubState &s, const std::string &band, const std::string &uid)
    {
        s._on_band_left(band, uid);
    }
    static void on_peer_connected(HubState &s, PeerEntry p) { s._on_peer_connected(std::move(p)); }
    static void on_peer_disconnected(HubState &s, const std::string &uid)
    {
        s._on_peer_disconnected(uid);
    }
    static void on_message_processed(HubState &s, const std::string &msg_type, std::size_t in,
                                     std::size_t out)
    {
    ...
    static void on_channel_access_opened(HubState &s, const std::string &channel_name)
    {
        s._on_channel_access_opened(channel_name);
    }
    static void on_channel_access_closed(HubState &s, const std::string &channel_name)
    {
    ...
    static std::uint64_t on_role_confirmed(HubState &s, const std::string &channel_name,
                                           const std::string &role_uid,
                                           std::uint64_t applied_version)
    {
        return s._on_role_confirmed(channel_name, role_uid, applied_version);
    }
    ...
    static std::size_t on_schemas_evicted_for_owner(HubState &s, const std::string &owner_uid)
    {
        return s._on_schemas_evicted_for_owner(owner_uid);
    }
    static ::pylabhub::schema::CitationOutcome
    validate_schema_citation(HubState &s, const ::pylabhub::hub::SchemaCitationInput &in)
    {
