 ...
 *
 * Exercises the two-pass timeout state machine (Connected -> Pending -> Disconnected)
...
{
    // Full Connected -> Pending -> Disconnected cycle, verified via
...
TEST_F(DatahubRoleStateMachineTest, PendingRecoversToConnected_OnHeartbeat)
{
    // Transient network flicker: role demoted Connected -> Pending, then recovers
    // via heartbeat. Counter pending_to_connected_total increments.
    auto proc = SpawnWorker("broker_role_state.pending_recovers_to_connected", {});
    ExpectWorkerOk(proc);
}

TEST_F(DatahubRoleStateMachineTest, StuckInPending_ReclaimedAfterPendingTimeout)
{
    // Role sends REG_REQ but never heartbeats (crash before first heartbeat).
