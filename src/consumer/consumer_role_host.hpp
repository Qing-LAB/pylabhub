 ...
 * state (short_tag, config, engine, RoleHostCore, RoleAPIBase,
...
class PYLABHUB_UTILS_EXPORT ConsumerRoleHost final : public scripting::RoleHostFrame
{
  ...
  private:
    ...
    /// Build the role's presence list (M9 step 2c).  Consumer returns a
    /// single Consumer-kind presence on in_hub/in_channel with both
