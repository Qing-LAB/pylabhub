...
ShmAttachOrchestrator::ShmAttachOrchestrator(AttachProtocolAcceptor &acceptor,
                                             IShmCapabilityProducer &transport, Config config)
    : acceptor_(acceptor), transport_(transport), config_(std::move(config))
{
    if (config_.channel_name.empty())
    {
        throw std::invalid_argument("ShmAttachOrchestrator: channel_name must be non-empty");
    }
    if (config_.producer_role_uid.empty())
    {
        throw std::invalid_argument("ShmAttachOrchestrator: producer_role_uid must be non-empty");
    }
    if (!config_.cache_lookup)
    {
        throw std::invalid_argument("ShmAttachOrchestrator: cache_lookup callback must be set");
    }
    if (!config_.broker_query)
    {
        throw std::invalid_argument("ShmAttachOrchestrator: broker_query callback must be set");
    }
}
...
ShmAttachOrchestrator::accept_and_serve_one(std::chrono::milliseconds timeout)
{
    ...
    {
        LOGGER_WARN("[shm_attach] handshake failed channel='{}': {}", config_.channel_name,
                    e.what());
    ...
    if (peer_fd < 0)
    {
        LOGGER_WARN("[shm_attach] accept returned valid AuthenticatedConsumer "
    ...
    try
    {
        broker_reply_opt = config_.broker_query(auth.consumer_pubkey_z85, auth.consumer_role_uid);
    }
    catch (const std::exception &e)
    {
        LOGGER_WARN("[shm_attach] broker_query callback threw "
                    "channel='{}' consumer_pubkey='{}' consumer_uid='{}': {} "
                    "— failing closed",
                    config_.channel_name, auth.consumer_pubkey_z85, auth.consumer_role_uid,
                    e.what());
        return Outcome::DeniedTransportFail;
    }
    if (!broker_reply_opt.has_value())
    {
        LOGGER_WARN("[shm_attach] broker_query returned nullopt "
                    "channel='{}' consumer_pubkey='{}' consumer_uid='{}' "
    ...
    const std::string status = broker_reply.value("status", std::string{});
    ...
    if (!broker_says_allowed && !broker_says_denied)
    {
        ...
        // closed.
        LOGGER_WARN("[shm_attach] broker reply has unexpected status='{}' "
                    "channel='{}' consumer_pubkey='{}' consumer_uid='{}' "
                    "— failing closed",
                    status, config_.channel_name, auth.consumer_pubkey_z85, auth.consumer_role_uid);
        return Outcome::DeniedTransportFail;
    }
    ...
    catch (const std::exception &e)
    {
        ...
        LOGGER_WARN("[shm_attach] cache_lookup callback threw "
    ...
    {
        LOGGER_WARN("[shm_attach] divergence broker=allowed cache=denied "
                    "channel='{}' consumer_pubkey='{}' consumer_uid='{}' "
    ...
    {
        LOGGER_WARN("[shm_attach] divergence broker=denied cache=allowed "
                    "channel='{}' consumer_pubkey='{}' consumer_uid='{}' "
    ...
    if (broker_says_allowed)
    {
        ...
        if (!sent)
        {
            LOGGER_WARN("[shm_attach] send_capability failed "
                        "channel='{}' consumer_pubkey='{}' consumer_uid='{}' "
                        "— failing closed",
                        config_.channel_name, auth.consumer_pubkey_z85, auth.consumer_role_uid);
            return Outcome::DeniedTransportFail;
        }
        LOGGER_INFO("[shm_attach] event=AttachAuthorized channel='{}' "
    ...
    LOGGER_INFO("[shm_attach] event=AttachDenied channel='{}' "
...
#error                                                                                             \
    "HEP-CORE-0041 §6.5: ShmAttachOrchestrator FreeBSD impl not yet shipped — task #259.  Builds against this platform must wait for the FreeBSD backend to land."
#elif defined(PYLABHUB_PLATFORM_APPLE)
#error                                                                                             \
    "HEP-CORE-0041 §6.5: ShmAttachOrchestrator macOS impl not yet shipped — task #260.  Builds against this platform must wait for the macOS backend to land."
#elif defined(PYLABHUB_PLATFORM_WIN64)
#error                                                                                             \
    "HEP-CORE-0041 §6.5: ShmAttachOrchestrator Windows impl not yet shipped — task #261.  Builds against this platform must wait for the Windows backend to land (named-pipe + DuplicateHandle)."
#else
#error                                                                                             \
    "HEP-CORE-0041 §6.5: ShmAttachOrchestrator: unknown platform.  Phase 1 supports Linux only; FreeBSD/macOS/Windows are Phases 2-3."
#endif
#endif
