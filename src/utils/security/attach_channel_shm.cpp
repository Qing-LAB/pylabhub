...
void recv_all_until(int fd, void *buf, std::size_t n,
                    std::chrono::steady_clock::time_point deadline, const char *side)
{
    ...
    while (got < n)
    {
        ...
        if (rem.count() == 0)
        {
            throw AttachProtocolTimeout(std::string{"AttachProtocol::"} + side +
                                        ": timeout waiting for " + std::to_string(n) +
              ...
              std::chrono::steady_clock::time_point deadline, const char *side)
{
    ...
    while (sent < n)
    {
        ...
        const ssize_t r = ::send(fd, p + sent, n - sent, MSG_NOSIGNAL | MSG_DONTWAIT);
        if (r < 0)
        {
            const int e = errno;
...
void ShmAttachChannel::send_frame(const nlohmann::json &frame,
                                  std::chrono::steady_clock::time_point deadline)
{
    const std::string s = frame.dump();
    if (s.size() > IAttachChannel::kMaxAttachFrameBytes)
    {
        throw std::runtime_error(std::string{"AttachProtocol::"} + side_ + ": frame too large (" +
...
nlohmann::json ShmAttachChannel::recv_frame(std::chrono::steady_clock::time_point deadline)
{
    std::uint8_t lb[4]{};
    ...
    {
        throw std::runtime_error(std::string{"AttachProtocol::"} + side_ + ": frame length " +
                                 std::to_string(len) + " exceeds DoS cap " +
                                 std::to_string(IAttachChannel::kMaxAttachFrameBytes));
    }
