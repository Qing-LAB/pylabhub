...
std::string b64_encode(std::span<const unsigned char> bin)
{
    const auto enc_max = sodium_base64_encoded_len(bin.size(), sodium_base64_VARIANT_ORIGINAL);
...
std::vector<unsigned char> b64_decode(const std::string &s)
{
    ...
    if (sodium_base642bin(out.data(), out.size(), s.data(), s.size(), nullptr, &bin_len, nullptr,
...
std::array<unsigned char, kRawKeyBytes> z85_pubkey_to_raw(const std::string &z85)
{
...
AttachProtocolAcceptor::AttachProtocolAcceptor(
    IShmCapabilityProducer &transport, uid_t expected_uid, std::string own_seckey_name,
    ObserverPubkeyAccessor broker_observer_pubkey_accessor)
    : transport_(transport), expected_uid_(expected_uid),
      ...
      broker_observer_pubkey_accessor_(std::move(broker_observer_pubkey_accessor))
{
    ...
    {
        throw std::invalid_argument("AttachProtocolAcceptor: own_seckey_name must be non-empty");
    }
}
...
AttachProtocolAcceptor::accept_one(std::chrono::milliseconds timeout)
{
    ...
    if (fd < 0)
    {
        throw std::runtime_error("AttachProtocolAcceptor: transport returned invalid peer fd");
    }
    ...
    const auto handshake_deadline = std::chrono::steady_clock::now() + timeout;
    ...
    {
        throw std::runtime_error("AttachProtocol::producer: SO_PEERCRED uid mismatch (expected " +
    ...
    const ProducerHandshakeResult phr = run_producer_handshake(
        channel, own_seckey_name_, broker_observer_pubkey_accessor_, handshake_deadline);
...
std::optional<int> initiate_consumer_handshake(const std::string &endpoint,
                                               ...
                                               bool require_mutual_auth)
{
    ...
    if (sock_path.empty())
    {
        throw std::invalid_argument("initiate_consumer_handshake: endpoint must be non-empty");
    }
    if (sock_path.size() >= sizeof(addr.sun_path))
    {
        throw std::invalid_argument("initiate_consumer_handshake: endpoint path too long");
    }
    if (self.role_uid.empty())
    {
        throw std::invalid_argument("initiate_consumer_handshake: self.role_uid must be non-empty");
    }
    if (self.pubkey_z85.size() != kZ85PubkeyChars)
    {
        throw std::invalid_argument("initiate_consumer_handshake: self.pubkey_z85 must be " +
                                    std::to_string(kZ85PubkeyChars) + " chars");
    }
    if (producer_pubkey_z85.size() != kZ85PubkeyChars)
    {
        throw std::invalid_argument("initiate_consumer_handshake: producer_pubkey_z85 must be " +
                                    std::to_string(kZ85PubkeyChars) + " chars");
    }
    ...
    if (::connect(fd, reinterpret_cast<const sockaddr *>(&addr), sizeof(addr)) == -1)
    {
        ...
        throw attach_make_errno_error("consumer", ("connect to '" + endpoint + "' failed").c_str(),
                                      e);
    }
    ...
    try
    {
        run_consumer_handshake(channel, self, producer_pubkey_z85, handshake_deadline,
                               require_mutual_auth);
    }
                       ...
                       std::chrono::steady_clock::time_point deadline)
{
    ...
    {
        throw std::invalid_argument("run_producer_handshake: own_seckey_name must be non-empty");
    }
    ...
    {
        throw std::runtime_error("AttachProtocol::producer: hello protocol_version mismatch");
    }
    ...
    std::string role_type = hello.value("role_type", std::string{"consumer"});
    if (role_type.empty())
        role_type = "consumer";
    if (role_type != "consumer" && role_type != "observer")
    {
        throw std::runtime_error("AttachProtocol::producer: hello role_type must be "
                                 "\"consumer\", \"observer\", or empty (got \"" +
                                 role_type + "\")");
    }
    const std::string pubkey_z85 = hello.value("pubkey_z85", std::string{});
    if (pubkey_z85.size() != kZ85PubkeyChars)
    {
        throw std::runtime_error("AttachProtocol::producer: hello pubkey_z85 must be " +
                                 std::to_string(kZ85PubkeyChars) + " chars (got " +
    ...
    if (role_type == "observer")
    {
        ...
        {
            throw std::runtime_error("AttachProtocol::producer: role_type=\"observer\" received "
        ...
        if (pubkey_z85.size() != trusted_broker_pubkey.size() ||
            !secure().memcmp_ct(
                std::span<const std::uint8_t>(
                    reinterpret_cast<const std::uint8_t *>(pubkey_z85.data()), pubkey_z85.size()),
                std::span<const std::uint8_t>(
                    reinterpret_cast<const std::uint8_t *>(trusted_broker_pubkey.data()),
                    trusted_broker_pubkey.size())))
        {
            throw std::runtime_error("AttachProtocol::producer: role_type=\"observer\" hello "
    ...
    const std::string response_b64 = hello.value("challenge_response_b64", std::string{});
    if (response_b64.empty())
    {
        throw std::runtime_error("AttachProtocol::producer: hello challenge_response_b64 missing");
    }
    ...
    {
        throw std::runtime_error("AttachProtocol::producer: challenge_response size mismatch "
                                 "(expected " +
    ...
    const bool verified =
        ...
        secure().memcmp_ct(std::span<const std::uint8_t>(plaintext, kChallengeBytes),
                           std::span<const std::uint8_t>(challenge, kChallengeBytes));
    secure().memzero(std::span<std::uint8_t>(plaintext, kChallengeBytes));

    if (!verified)
    {
        throw std::runtime_error("AttachProtocol::producer: challenge-response verification "
    ...
    // ── 8. Optional mutual-auth Frame 3 (HEP-CORE-0041 §D4.5, #262) ──────
    {
        ...
        try
        {
            cons_nonce_b64 = hello.value("consumer_nonce_b64", std::string{});
            cons_challenge_b64 = hello.value("consumer_challenge_b64", std::string{});
        }
        catch (const nlohmann::json::exception &e)
        {
            throw std::runtime_error(std::string("AttachProtocol::producer: consumer mutual-auth "
                                                 "field wrong type — ") +
                                     e.what());
        }
        if (!cons_nonce_b64.empty() || !cons_challenge_b64.empty())
        {
                ...
                cons_challenge_vec.size() != kChallengeBytes)
            {
                throw std::runtime_error(
                    ...
                    std::to_string(kNonceBytes) + ", challenge=" + std::to_string(kChallengeBytes) +
                    ")");
            }
            ...
            const std::size_t proof_written = secure().box_encrypt_using(
                ...
                std::span<const std::uint8_t>(cons_challenge_vec.data(), kChallengeBytes),
                std::span<std::uint8_t>(proof_cipher.data(), proof_cipher.size()));
            if (proof_written != proof_cipher.size())
            {
                throw std::runtime_error("AttachProtocol::producer: box_encrypt_using for "
                                         "Frame 3 proof failed (secret " +
                                         own_seckey_name +
                                         " missing from KeyStore, or sodium error)");
            }

            const std::string_view producer_pk_z85 = secure().keys().pubkey(own_seckey_name);
            nlohmann::json proof;
            proof["producer_pubkey_z85"] = std::string(producer_pk_z85);
            proof["proof_response_b64"] = b64_encode({proof_cipher.data(), proof_cipher.size()});
...
void run_consumer_handshake(IAttachChannel &channel, const ConsumerAuthMaterial &self,
                            const std::string &producer_pubkey_z85,
                            std::chrono::steady_clock::time_point deadline,
                            bool require_mutual_auth)
{
    if (self.role_uid.empty())
    {
        throw std::invalid_argument("run_consumer_handshake: self.role_uid must be non-empty");
    }
    if (self.pubkey_z85.size() != kZ85PubkeyChars)
    {
        throw std::invalid_argument("run_consumer_handshake: self.pubkey_z85 must be " +
                                    std::to_string(kZ85PubkeyChars) + " chars");
    }
    if (producer_pubkey_z85.size() != kZ85PubkeyChars)
    {
        throw std::invalid_argument("run_consumer_handshake: producer_pubkey_z85 must be " +
                                    std::to_string(kZ85PubkeyChars) + " chars");
    }
    if (self.own_seckey_name.empty())
    {
    ...
    try
    {
        protocol_version = challenge_in.value("protocol_version", std::string{});
        nonce_b64_field = challenge_in.value("nonce_b64", std::string{});
        challenge_b64_field = challenge_in.value("challenge_b64", std::string{});
    }
    catch (const nlohmann::json::exception &e)
    {
        throw std::runtime_error(std::string("AttachProtocol::consumer: Frame 1 field wrong "
                                             "type — ") +
                                 e.what());
    }
    if (protocol_version != kProtocolVersion)
    {
        throw std::runtime_error("AttachProtocol::consumer: challenge protocol_version mismatch");
    }
    const auto nonce_dec = b64_decode(nonce_b64_field);
    if (nonce_dec.size() != kNonceBytes)
    {
        throw std::runtime_error("AttachProtocol::consumer: nonce size mismatch (expected " +
    ...
    {
        throw std::runtime_error("AttachProtocol::consumer: challenge size mismatch (expected " +
    ...
    {
        throw std::runtime_error("AttachProtocol::consumer: box_encrypt_using failed (secret '" +
                                 self.own_seckey_name +
                                 "' missing from KeyStore, or sodium error)");
    }
    ...
    if (require_mutual_auth)
    {
        ...
        hello["consumer_nonce_b64"] = b64_encode({consumer_nonce.data(), kNonceBytes});
        hello["consumer_challenge_b64"] = b64_encode({consumer_challenge.data(), kChallengeBytes});
    }
    ...
    if (require_mutual_auth)
    {
        ...
        try
        {
            proof_producer_pk = proof.value("producer_pubkey_z85", std::string{});
            proof_response_b64 = proof.value("proof_response_b64", std::string{});
        }
        catch (const nlohmann::json::exception &e)
        {
            throw std::runtime_error(std::string("AttachProtocol::consumer: Frame 3 field wrong "
                                                 "type (attach_producer_not_authenticated) — ") +
                                     e.what());
        }
        if (proof_producer_pk.size() != kZ85PubkeyChars || proof_response_b64.empty())
        {
            throw std::runtime_error("AttachProtocol::consumer: Frame 3 malformed "
                                     "(attach_producer_not_authenticated) — expected "
                                     "producer_pubkey_z85 + proof_response_b64");
        }
        if (proof_producer_pk != producer_pubkey_z85)
        {
            throw std::runtime_error("AttachProtocol::consumer: producer_pubkey_z85 mismatch "
                                     "(attach_producer_not_authenticated) — expected '" +
        ...
        {
            throw std::runtime_error("AttachProtocol::consumer: proof_response_b64 size mismatch "
                                     "(attach_producer_not_authenticated) — expected " +
                                     std::to_string(kChallengeBytes + kMacBytes) + " bytes, got " +
        ...
        const std::size_t proof_decoded = secure().box_decrypt_using(
            ...
            std::span<const std::uint8_t>(proof_cipher.data(), proof_cipher.size()),
            std::span<std::uint8_t>(proof_plain.data(), kChallengeBytes));
        const bool proof_verified =
            (proof_decoded == kChallengeBytes) &&
            secure().memcmp_ct(
                std::span<const std::uint8_t>(proof_plain.data(), kChallengeBytes),
                std::span<const std::uint8_t>(consumer_challenge.data(), kChallengeBytes));
        secure().memzero(std::span<std::uint8_t>(proof_plain.data(), kChallengeBytes));
        if (!proof_verified)
        {
            throw std::runtime_error("AttachProtocol::consumer: Frame 3 proof verification "
...
#error                                                                                             \
    "HEP-CORE-0041 AttachProtocol: only Linux is implemented in substep 1c. FreeBSD/macOS/Windows L1 backends are #error'd (tasks #259/#260/#261); the L2 here would also need AcceptedPeer-variant branching when those land. Refusing to compile until implemented for this platform."
