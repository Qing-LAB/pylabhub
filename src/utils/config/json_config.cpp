...
bool JsonConfig::reload(std::error_code *err_code) noexcept
{
    try
    {
        ...
        FileLock fLock(pImpl->configPath);
        if (!fLock.valid())
        {
            if (err_code != nullptr)
            {
                                               ...
                                               std::error_code *err_code) noexcept
{
    try
    {
        ...
        // at the same time.
        FileLock file_lock(pImpl->configPath);
        if (!file_lock.valid())
        {
            if (err_code != nullptr)
            {
...
nlohmann::json &JsonConfig::WriteLock::json() noexcept
{
    if (d_ == nullptr || d_->owner == nullptr)
    {
        thread_local nlohmann::json dummy = nlohmann::json::object();
        return dummy;
    }
...
namespace
{
void do_jsonconfig_startup(const char *arg, void * /*userdata*/)
{
    (void)arg;
    g_jsonconfig_initialized.store(true, std::memory_order_release);
}
void do_jsonconfig_shutdown(const char *arg, void * /*userdata*/)
{
    (void)arg;
