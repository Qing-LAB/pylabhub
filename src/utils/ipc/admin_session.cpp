...
std::optional<AdminSessionFacts> deserialize_facts(std::string_view pt)
{
    ...
    if (!j.is_object() || !j.contains("l") || !j.contains("p") ||
        !j.contains("r") || !j.contains("t"))
        return std::nullopt;
...
std::optional<AdminSessionFacts> open_session_id(std::string_view sealed_hex)
{
    ...
    const std::size_t got = sec::secure().secretbox_decrypt(
        pt.data(), pt.size(),
        reinterpret_cast<const std::uint8_t *>(blob.data() + kNonceBytes), ct_len,
    ...
    return deserialize_facts(
        std::string_view(reinterpret_cast<const char *>(pt.data()), got));
}

std::optional<AdminSessionFacts>
                  ...
                  std::string_view observed_routing_id)
{
    ...
    if (!facts)
        return std::nullopt;
    if (facts->peer_address != observed_peer_address ||
        facts->routing_id != observed_routing_id)
...
{
    return facts.label + "@" + facts.peer_address + "#" +
           std::to_string(facts.issued_at_ms);
}
