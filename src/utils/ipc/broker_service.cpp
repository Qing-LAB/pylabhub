...
nlohmann::json channel_to_json(const pylabhub::hub::ChannelEntry &c)
{
    nlohmann::json j;
    j["name"]                = c.name;
    j["status"]              = pylabhub::hub::to_string(c.status);
...
nlohmann::json role_to_json(const pylabhub::hub::RoleEntry &r)
{
    nlohmann::json j;
    j["uid"]            = r.uid;
    j["name"]           = r.name;
    j["role_tag"]       = r.role_tag;
    j["state"]          = pylabhub::hub::to_string(r.state);
...
nlohmann::json band_to_json(const pylabhub::hub::BandEntry &b)
{
    nlohmann::json j;
    j["name"] = b.name;
    ...
    for (const auto &m : b.members)
    {
        ...
        // joined_at is a steady_clock time_point — not directly format-able
        // as wall-clock.  Emit milliseconds since join instead so consumers
        // have a relative-age signal without us guessing wall-clock skew.
...
nlohmann::json peer_to_json(const pylabhub::hub::PeerEntry &p)
{
    nlohmann::json j;
    j["uid"]             = p.uid;
    j["endpoint"]        = p.endpoint;
    j["state"]           = pylabhub::hub::to_string(p.state);
    j["pubkey_z85"]      = p.pubkey_z85;
    j["relay_channels"]  = p.relay_channels;
    return j;
}

nlohmann::json broker_counters_to_json(const pylabhub::hub::BrokerCounters &c)
{
    nlohmann::json j;
