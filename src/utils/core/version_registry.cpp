...
static_assert(pylabhub::version::kShmMajor == pylabhub::hub::detail::HEADER_VERSION_MAJOR,
              "plh_version_registry.hpp: kShmMajor drifted from "
              "data_block.hpp HEADER_VERSION_MAJOR");
static_assert(pylabhub::version::kShmMinor == pylabhub::hub::detail::HEADER_VERSION_MINOR,
...
std::string version_info_string()
{
    ...
    return fmt::format("pylabhub {} (lib={}.{}.{}, shm={}.{}, broker={}.{}, zmq={}.{}, "
                       "script={}.{}, engine={}.{}, config={}.{}, python_rt={}{}{})",
                       PYLABHUB_RELEASE_VERSION, v.library_major, v.library_minor,
                       v.library_rolling, v.shm_major, v.shm_minor, v.broker_proto_major,
                       v.broker_proto_minor, v.zmq_frame_major, v.zmq_frame_minor,
                       v.script_api_major, v.script_api_minor, v.script_engine_major,
                       v.script_engine_minor, v.config_major, v.config_minor,
                       PYLABHUB_PYTHON_RUNTIME_VERSION, bid ? ", build=" : "", bid ? bid : "");
}
...
std::string version_info_json()
{
    const auto v = current();
    const char *bid = build_id();
    return fmt::format(R"({{"release":"{}","library":"{}.{}.{}","python_runtime":"{}",)"
                       ...
                       R"(}})",
                       PYLABHUB_RELEASE_VERSION, v.library_major, v.library_minor,
                       v.library_rolling, PYLABHUB_PYTHON_RUNTIME_VERSION, v.shm_major, v.shm_minor,
                       v.broker_proto_major, v.broker_proto_minor, v.zmq_frame_major,
                       v.zmq_frame_minor, v.script_api_major, v.script_api_minor,
                       v.script_engine_major, v.script_engine_minor, v.config_major, v.config_minor,
                                  ...
                                  const char *remote_build_id) noexcept
{
    ...
    auto check_major = [&](const char *name, auto rem, auto loc, bool &flag)
    {
        if (rem != loc)
        {
            ...
            v.result.message += fmt::format("{} major {} != {}; ", name, static_cast<unsigned>(rem),
    ...
    check_major("shm", remote.shm_major, cur.shm_major, v.result.major_mismatch.shm);
    ...
    check_major("config", remote.config_major, cur.config_major, v.result.major_mismatch.config);
    ...
    if (remote_build_id != nullptr)
    {
        ...
        if (cur_bid == nullptr || std::strcmp(remote_build_id, cur_bid) != 0)
        {
            ...
            v.result.message += fmt::format("build_id {} != {}; ", remote_build_id,
    ...
    auto note_minor = [&](const char *name, auto rem, auto loc, bool &flag)
    {
        ...
        {
            flag = true;
            v.minor_notes.push_back(fmt::format(
                "{} minor {} != {}", name, static_cast<unsigned>(rem), static_cast<unsigned>(loc)));
            v.minor_axes.push_back(name);
        }
    };
    note_minor("library", remote.library_minor, cur.library_minor, v.result.minor_mismatch.library);
    note_minor("shm", remote.shm_minor, cur.shm_minor, v.result.minor_mismatch.shm);
    ...
    note_minor("config", remote.config_minor, cur.config_minor, v.result.minor_mismatch.config);
...
AbiCheckResult check_abi(const ComponentVersions &exp, const char *exp_build_id) noexcept
{
...
AbiPeerVerdict classify_peer_verdict(const AbiCheckResult &v)
{
    ...
    if (!v.compatible)
    {
        ...
        const bool only_build_id = v.major_mismatch.build_id && !v.major_mismatch.library &&
                                   !v.major_mismatch.shm && !v.major_mismatch.broker_proto &&
                                   !v.major_mismatch.zmq_frame && !v.major_mismatch.script_api &&
                                   !v.major_mismatch.script_engine && !v.major_mismatch.config;
        if (only_build_id)
        {
            ...
            // is correct.
            out.kind = out.minor_axes.empty() ? AbiPeerVerdict::Kind::BuildOnly
...
ComponentVersions from_json_object(const nlohmann::json &j)
{
    if (!j.is_object())
    {
        throw std::invalid_argument("abi_fingerprint: expected JSON object");
    }
    ...
    auto req_axis_u8 = [&](const char *key) -> uint8_t
    {
        ...
        {
            throw std::invalid_argument(fmt::format("abi_fingerprint: missing or non-unsigned "
                                                    "axis field '{}'",
                                                    key));
        }
        ...
        {
            throw std::invalid_argument(fmt::format("abi_fingerprint: axis '{}' value {} exceeds "
                                                    "uint8_t range",
                                                    key, raw));
        }
