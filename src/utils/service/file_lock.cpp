...
static void open_and_lock(FileLockImpl *pImpl, bool is_directory, bool blocking,
                          std::optional<std::chrono::milliseconds> timeout);
static bool prepare_lock_path(FileLockImpl *pImpl, bool is_directory);
...
static bool run_os_lock_loop(FileLockImpl *pImpl, bool blocking,
                             std::optional<std::chrono::milliseconds> timeout);
...
// Public Methods
FileLock::FileLock(const std::filesystem::path &path, bool is_directory, bool blocking) noexcept
    : pImpl(new (std::nothrow) FileLockImpl)
{
    ...
    {
        pImpl->path = path;
        open_and_lock(pImpl.get(), is_directory, blocking, std::nullopt);
    }
}

FileLock::FileLock(const std::filesystem::path &path, bool is_directory,
    ...
    : pImpl(new (std::nothrow) FileLockImpl)
{
    ...
    {
        pImpl->path = path;
        open_and_lock(pImpl.get(), is_directory, /*blocking=*/true, timeout);
    }
}
...
std::optional<FileLock> FileLock::try_lock(const std::filesystem::path &path, bool is_directory,
                                           bool blocking) noexcept
{
    ...
    open_and_lock(lock.pImpl.get(), is_directory, blocking, std::nullopt);
                                           ...
                                           std::chrono::milliseconds timeout) noexcept
{
    ...
    open_and_lock(lock.pImpl.get(), is_directory, /*blocking=*/true, timeout);
                          ...
                          std::optional<std::chrono::milliseconds> timeout)
{
    ...
    if (!prepare_lock_path(pImpl, is_directory))
    {
        return;
    }
    if (!acquire_process_local_lock(pImpl, blocking, timeout))
    {
        return;
    }
    ...
    if (!run_os_lock_loop(pImpl, blocking, timeout))
    {
        rollback_process_local_lock(pImpl);
        return;
    }
...
static bool prepare_lock_path(FileLockImpl *pImpl, bool is_directory)
{
    auto lockpath = FileLock::get_expected_lock_fullname_for(pImpl->path, is_directory);
    if (lockpath.empty())
    {
