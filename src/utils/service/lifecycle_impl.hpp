...
#include <atomic>             // For std::atomic<bool> module status
#include <chrono>             // For std::chrono::milliseconds, steady_clock
#include <condition_variable> // For std::condition_variable
#include <cstdint>            // For std::uint8_t
#include <string_view>        // For std::string_view (module labels)
 ...
 */
/// @param label Module name, reported by the WORKER thread on entry and on
///        exit.  A label with an ENTER and no matching EXIT identifies the
///        module that overran — which is why the worker reports rather than
///        the caller: on timeout the caller detaches and returns, so it can
///        only ever record that the module overran, never whether it
///        eventually finished.
ShutdownOutcome timedShutdown(const std::function<void()> &func, std::chrono::milliseconds timeout,
                              std::string_view label);


/// Size of the stack buffer each report site formats into.  One
/// constant, because a step marker is a step marker wherever it is
/// written; earlier revisions of this file had 256 in four places and 320
/// in a fifth for no reason anyone could defend.
inline constexpr std::size_t kCriticalReportLineBytes = 256;

/// Forward a formatted step marker to the debug module's last-resort
/// trace, clamping `n` to what actually fit.
///
/// Exists so call sites can pass `fmt::format_to_n(...).size` directly:
/// that value is the size the output WOULD have needed, which is larger
/// than the buffer when the text was truncated, so handing it on unclamped
/// would read past the end. Getting that wrong at one of five call sites
/// is exactly the sort of thing a shared helper prevents.
