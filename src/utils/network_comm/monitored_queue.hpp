 ...
 * @brief MonitoredQueue<T> — bounded thread-safe command queue with optional backpressure
 * monitoring.
 *
...
template <typename T> class MonitoredQueue
{
  public:
    struct Config
    {
    ...
    void set_on_push_signal(std::function<void()> cb) noexcept { on_push_signal_ = std::move(cb); }
    ...
    template <typename Sender> void drain(Sender &&sender)
    {
        size_t depth_before;
        {
            std::lock_guard lock(mu_);
        ...
        if (!cfg_.fire_and_forget)
            run_check_(depth_before);
    }
    ...
    void run_check_(size_t depth_before)
    {
        ...
        if (depth_before > 0 && depth_before >= depth_at_last_check_)
        {
            ...
            auto elapsed = static_cast<int>(
                std::chrono::duration_cast<std::chrono::milliseconds>(now - backpressure_start_)
                    .count());
            if (on_warn_)
                on_warn_(depth_before, elapsed);
