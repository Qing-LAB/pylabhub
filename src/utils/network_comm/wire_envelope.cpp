...
std::string compute_envelope_hash_impl(std::string_view identity,
...
void stamp_and_append_body_frames(std::string_view identity,
                                    std::string_view msg_type,
                                    std::string_view correlation_id,
                                    nlohmann::json &body,
                                    zmq::multipart_t &out)
...
zmq::multipart_t
WireEnvelope::build_dealer_send(std::string_view identity,
                                 std::string_view msg_type,
                                 std::string_view correlation_id,
                                 nlohmann::json    body)
{
    ...
    if (identity.empty())
    {
        throw std::invalid_argument(
            "WireEnvelope::build_dealer_send: identity empty "
    ...
    {
        throw std::invalid_argument(
            "WireEnvelope::build_dealer_send: correlation_id > 64 bytes");
    }
    if (correlation_id.empty() && !is_notify_msg_type(msg_type))
    {
        // I-CORRELATION-STABLE: empty allowed only on NOTIFY.
        throw std::invalid_argument(
            "WireEnvelope::build_dealer_send: empty correlation_id on "
    ...
    {
        throw std::invalid_argument(
            "WireEnvelope::build_dealer_send: body must be a JSON object");
    }

    zmq::multipart_t frames;
    stamp_and_append_body_frames(identity, msg_type, correlation_id, body,
                                  frames);
    return frames;
}

zmq::multipart_t
WireEnvelope::build_router_send(std::string_view target_identity,
                                 std::string_view msg_type,
                                 std::string_view correlation_id,
                                 nlohmann::json    body)
{
    ...
    {
        throw std::invalid_argument(
            "WireEnvelope::build_router_send: target_identity empty");
    }
    auto frames = build_dealer_send(target_identity, msg_type, correlation_id,
                                     std::move(body));
...
bool parse_common(std::string_view identity,
                    std::string_view msg_type,
                    std::string_view correlation_id,
                    std::string_view body_str,
                    WireEnvelope::Impl &out,
                    ParseError         *err_out)
{
    if (msg_type.empty())
    {
        if (err_out) *err_out = ParseError::msg_type_empty;
        return false;
    }
    if (msg_type.size() > kMaxMsgTypeLen)
    {
        if (err_out) *err_out = ParseError::msg_type_too_long;
        return false;
    }
    if (correlation_id.size() > kMaxCorrelationIdLen)
    {
        if (err_out) *err_out = ParseError::correlation_too_long;
        return false;
    }
    if (correlation_id.empty() && !is_notify_msg_type(msg_type))
    {
        // I-CORRELATION-STABLE.
        if (err_out) *err_out = ParseError::correlation_missing;
        return false;
    }
    ...
    {
        if (err_out) *err_out = ParseError::body_not_json;
        return false;
    }
    if (!body.is_object())
    {
        if (err_out) *err_out = ParseError::body_not_object;
        return false;
    }
    ...
    {
        if (err_out) *err_out = ParseError::envelope_hash_missing;
        return false;
    }
    const auto received_hash =
        body.at(std::string{kEnvelopeHashKey}).get<std::string>();
    const auto expected_hash =
        compute_envelope_hash_impl(identity, msg_type, correlation_id);
    if (received_hash != expected_hash)
    {
        if (err_out) *err_out = ParseError::envelope_hash_mismatch;
        return false;
    }
...
std::optional<WireEnvelope>
WireEnvelope::parse_router_recv(zmq::multipart_t &&msg, ParseError *err_out)
{
    // ROUTER receive: [identity, 'C', msg_type, correlation_id, body]
    if (msg.size() != 5)
    {
        if (err_out) *err_out = ParseError::frame_count;
        return std::nullopt;
    }
    ...
    if (marker_frame.size() != 1 ||
        *static_cast<const std::uint8_t *>(marker_frame.data())
            != kFrameTypeControl)
    {
        if (err_out) *err_out = ParseError::frame_type_marker;
        return std::nullopt;
    }

    WireEnvelope env;
    if (!parse_common(msg_as_view(identity_frame),
                       msg_as_view(msg_type_frame),
                       msg_as_view(correlation_frame),
                       msg_as_view(body_frame), *env.pImpl, err_out))
    {
        return std::nullopt;
    }
    return env;
...
std::optional<WireEnvelope>
WireEnvelope::parse_dealer_recv(zmq::multipart_t &&msg,
                                 ...
                                 ParseError        *err_out)
{
    ...
    WireEnvelope env;
    if (!parse_common(dealer_own_routing_id,
                       msg_as_view(msg_type_frame),
                       msg_as_view(correlation_frame),
                       msg_as_view(body_frame), *env.pImpl, err_out))
    {
