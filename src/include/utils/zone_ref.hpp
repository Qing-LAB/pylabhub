...
template <typename FlexZoneT, bool IsMutable> class ZoneRef
{
  public:
     ...
     */
    explicit ZoneRef(DataBlockProducer *producer)
        requires IsMutable
        : m_producer(producer)
    {
        if (!producer)
     ...
     */
    explicit ZoneRef(DataBlockConsumer *consumer)
        requires(!IsMutable)
        : m_consumer(consumer)
    {
        if (!consumer)
    ...
    [[nodiscard]] value_type &get()
        requires(!std::is_void_v<FlexZoneT>)
    {
        ...
        {
            throw std::runtime_error("ZoneRef::get(): flexible zone size (" +
     ...
     * @throws std::runtime_error if no flexible zone configured
     */
    [[nodiscard]] const FlexZoneT &get() const
        requires(!std::is_void_v<FlexZoneT>)
    {
        auto raw = raw_access();
     ...
     * @return Mutable span of bytes covering the flexible zone (empty if not configured or
     * moved-from)
     *
     ...
     */
    [[nodiscard]] std::span<std::byte> raw_access() noexcept
        requires IsMutable
    {
        if (!m_producer)
        {
     ...
     * @return Const span of bytes covering the flexible zone (empty if not configured or
     * moved-from)
     */
     ...
     */
    [[nodiscard]] bool has_zone() const noexcept { return !raw_access().empty(); }

    /**
     * @brief Get flexible zone size in bytes
     * @return Size of flexible zone (0 if not configured)
     */
    [[nodiscard]] size_t size() const noexcept { return raw_access().size(); }
