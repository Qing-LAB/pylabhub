...
extern "C"
{
                                                                         ...
                                                                         uint32_t slot_index)
    {
        ...
        try
        {
            ...
            // State-aware release: if the zombie writer already committed data (COMMITTED),
            // preserve the slot so consumers can still read it. Only reset to FREE when
            // the writer died mid-write (WRITING or other non-COMMITTED state).
            auto current_state = rw_state->slot_state.load(std::memory_order_acquire);
            if (current_state != pylabhub::hub::SlotRWState::SlotState::COMMITTED)
            {
                rw_state->slot_state.store(pylabhub::hub::SlotRWState::SlotState::FREE,
                                           std::memory_order_release);
            }
        ...
        try
        {
                ...
                pylabhub::hub::ChecksumType::Unset)
            {
                ...
                if (total_written > 0)
                {
                    ...
                    // When the ring buffer has wrapped (total_written >= slot_count) every slot
                    // contains committed data and all slots must be checked.  When it has not
                    // wrapped, only slots 0..commit_idx%slot_count are committed.
                    const bool ring_full = (total_written >= slot_count);
                    for (uint32_t i = 0; i < slot_count; ++i)
                    {
                        if (ring_full || i <= (commit_idx % slot_count))
                        {
