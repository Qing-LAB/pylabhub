...
bool SharedSpinLock::try_lock_for(int timeout_ms)
{
    ...
    // H4: acquire on owner_pid synchronizes with the release store in unlock() so this
    ...
    // H5: release on owner_tid so that the acquire in the recursion check above can see
    //     the freshly stored TID and not a stale zero or previous owner's TID.
    m_state->owner_tid.store(my_tid, std::memory_order_release);
    m_state->recursion_count.store(1, std::memory_order_relaxed);
    return true;
}
