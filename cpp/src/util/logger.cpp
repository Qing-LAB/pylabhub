// logger.cpp
// #define LOGGER_EXPORTS // this is only needed for windows the cmake should handle this properly
...
static int level_to_sysLOGGER_priority(Logger::Level lvl)
{
    switch (lvl)
    {
    case Logger::Level::TRACE:
...
{
    if (static_cast<int>(lvl) < pImpl->level.load(std::memory_order_relaxed))
        return;
    ...
    {
        syslog(level_to_sysLOGGER_priority(lvl), "%s", (header + body).c_str());
    }
    else
    {
...
{
    if (static_cast<int>(Logger::Level::TRACE) < pImpl->level.load(std::memory_order_relaxed))
        return;
    va_list ap;
    va_start(ap, fmt);
    log(Logger::Level::TRACE, fmt, ap);
    va_end(ap);
}
void Logger::debug(const char *fmt, ...)
{
    if (static_cast<int>(Logger::Level::DEBUG) < pImpl->level.load(std::memory_order_relaxed))
        return;
    va_list ap;
    va_start(ap, fmt);
    log(Logger::Level::DEBUG, fmt, ap);
    va_end(ap);
}
void Logger::info(const char *fmt, ...)
{
    if (static_cast<int>(Logger::Level::INFO) < pImpl->level.load(std::memory_order_relaxed))
        return;
    va_list ap;
    va_start(ap, fmt);
    log(Logger::Level::INFO, fmt, ap);
    va_end(ap);
}
void Logger::warn(const char *fmt, ...)
{
    if (static_cast<int>(Logger::Level::WARNING) < pImpl->level.load(std::memory_order_relaxed))
        return;
    va_list ap;
    va_start(ap, fmt);
    log(Logger::Level::WARNING, fmt, ap);
    va_end(ap);
}
