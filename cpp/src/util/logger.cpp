...
struct Logger::Impl {
    Destination dest = Destination::CONSOLE;
#if defined(PLATFORM_WIN64)
    ...
    std::chrono::steady_clock::time_point last_stderr_notice = std::chrono::steady_clock::now() - std::chrono::seconds(10);
...
size_t Logger::max_log_line_length() const {
...
struct Logger::ImplAccessor {
    Impl *p;
    explicit ImplAccessor(Impl *p_) : p(p_) {}
    void record_write_error(int errcode, const std::string &msg) {
        ...
        if (p->write_error_callback) {
            try { p->write_error_callback(msg); } catch(...) {}
        }
...
extern "C" LOGGER_API Logger *get_global_logger() { return &Logger::instance(); }
...
bool Logger::init_file(const std::string &path, bool use_flock, int mode) {
    ...
    if (wpath.empty()) { pImpl->last_errno.store(GetLastError()); return false; }
    HANDLE h = CreateFileW(wpath.c_str(), FILE_APPEND_DATA | GENERIC_WRITE, FILE_SHARE_READ, nullptr, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
    if (h == INVALID_HANDLE_VALUE) { pImpl->last_errno.store(GetLastError()); return false; }
    SetFilePointer(h, 0, nullptr, FILE_END);
    if (pImpl->file_handle != INVALID_HANDLE_VALUE) CloseHandle(pImpl->file_handle);
    pImpl->file_handle = h;
    pImpl->file_path = path; pImpl->use_flock = use_flock; pImpl->dest = Destination::FILE;
    return true;
#else
    if (pImpl->file_fd != -1) { ::close(pImpl->file_fd); pImpl->file_fd = -1; }
    int fd = ::open(path.c_str(), O_CREAT | O_WRONLY | O_APPEND, static_cast<mode_t>(mode));
    if (fd == -1) { pImpl->last_errno.store(errno); return false; }
    pImpl->file_fd = fd; pImpl->file_path = path; pImpl->use_flock = use_flock; pImpl->dest = Destination::FILE;
    return true;
#endif
}
...
bool Logger::init_eventlog(const wchar_t *source_name) {
    ...
    if (!h) { pImpl->last_errno.store(GetLastError()); return false; }
    pImpl->evt_handle = h; pImpl->dest = Destination::EVENTLOG; return true;
#else
    (void)source_name; return false;
#endif
}

void Logger::set_destination(Destination dest) { std::lock_guard<std::mutex> g(pImpl->mtx); pImpl->dest = dest; }
void Logger::shutdown() {
    ...
    closelog();
#endif
    pImpl->dest = Destination::CONSOLE;
}
...
void Logger::set_write_error_callback(std::function<void(const std::string &)> cb) { std::lock_guard<std::mutex> g(pImpl->mtx); pImpl->write_error_callback = std::move(cb); }

int Logger::last_errno() const { return pImpl->last_errno.load(); }
int Logger::last_write_error_code() const { return pImpl->last_write_errcode.load(); }
std::string Logger::last_write_error_message() const { std::lock_guard<std::mutex> g(pImpl->mtx); return pImpl->last_write_errmsg; }
int Logger::write_failure_count() const { return pImpl->write_failure_count.load(); }
...
void Logger::write_formatted(Level lvl, std::string &&body) noexcept {
    ...
    std::lock_guard<std::mutex> g(pImpl->mtx);
    ImplAccessor acc(pImpl);

    if (pImpl->dest == Destination::CONSOLE) {
    ...
    if (pImpl->dest == Destination::FILE) {
#if defined(PLATFORM_WIN64)
        if (pImpl->file_handle != INVALID_HANDLE_VALUE) {
            if (pImpl->use_flock) {
                OVERLAPPED ov = {};
                if (!LockFileEx(pImpl->file_handle, LOCKFILE_EXCLUSIVE_LOCK, 0, 0xFFFFFFFF, 0xFFFFFFFF, &ov)) {
                ...
                if (!ok) acc.record_write_error(GetLastError(), "WriteFile failed");
                else if (pImpl->fsync_per_write.load()) if (!FlushFileBuffers(pImpl->file_handle)) acc.record_write_error(GetLastError(), "FlushFileBuffers failed");
                OVERLAPPED ov2 = {};
        ...
        if (pImpl->file_fd != -1) {
            ...
            if (pImpl->fsync_per_write.load()) {
                if (::fsync(pImpl->file_fd) != 0) acc.record_write_error(errno, strerror(errno));
            }
            if (pImpl->use_flock) flock(pImpl->file_fd, LOCK_UN);
        } else {
            size_t wrote = fwrite(full.c_str(), 1, full.size(), stderr);
            if (wrote != full.size()) acc.record_write_error(errno, strerror(errno));
            fflush(stderr);
        }
#endif
        return;
    ...
    if (pImpl->dest == Destination::SYSLOG) {
#if defined(PLATFORM_WIN64)
    ...
    if (pImpl->dest == Destination::EVENTLOG) {
#if defined(PLATFORM_WIN64)
        if (pImpl->evt_handle) {
...
void Logger::log(const char *fmt, ...) {
    ...
    va_list ap;
    va_start(ap, fmt);
    // Create a string via vsnprintf as fallback; this keeps backward-compatibility but is less efficient.
