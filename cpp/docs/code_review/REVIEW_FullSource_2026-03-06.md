...
| 5 | MEDIUM | Race Condition | `hubshell.cpp` | Broker callbacks reference `HubScript::get_instance()` before dynamic module loaded | FALSE POSITIVE — callbacks queue events via `on_hub_peer_connected/disconnected/message()`; hub_thread_ only dequeues events after `script_loaded=true`; startup window is safe by queue ordering. |
| 6 | MEDIUM | Python/GIL | `hub_script.cpp:392` | `script_module.is_none()` pointer check without GIL is unsafe | FALSE POSITIVE — `script_module` is a hub_thread_-local variable (never shared across threads); `is_none()` is a single pointer comparison with no Python C API calls. Documented at hub_script.cpp:407-409. |
...
| 8 | MEDIUM | Redundant Code | `broker_service.cpp` / `messenger_internal.hpp` | Hex encode/decode duplicated between broker and messenger | ✅ FIXED 2026-03-11 — all 6 sites (messenger_internal.hpp, broker_service.cpp, schema_library.cpp, consumer/processor/producer script hosts) delegate to `format_tools::bytes_to_hex` / `bytes_from_hex`; inline kHex[] loops removed |
...
| 26 | MEDIUM | Protocol | `messenger_protocol.cpp:779-812` | Fire-and-forget handlers don't validate JSON structure before forwarding | FALSE POSITIVE — lines 779-812 are the client-side SEND path (building JSON, not parsing received messages). The broker's receive-side (`handle_channel_notify_req`, `handle_channel_broadcast_req`) uses `.value()` with safe defaults and validates mandatory fields (`target_channel.empty()` guard). Clients are authenticated before sending. |
...
| 33 | MEDIUM | Performance | `data_block_slot_ops.cpp:92,99,132,186` | Excessive `seq_cst` fences where `acquire`/`release` suffices (~10-20% ARM64 penalty) | ACCEPTED 2026-03-12 — fences are CORRECT and NECESSARY. Reader and writer synchronise across two *different* variables (`reader_count` and `slot_state`); acquire/release only orders ops on the same variable's modification order. The paired seq_cst fences (reader Step 3 + writer drain-loop line 134) create the required total ordering: whichever fence comes first, the other side sees the correct state in its subsequent check. Replacing with acq/rel would break the TOCTTOU protocol. File-level "Do not reorder without reviewing the HEP and tests" warning is in place (line 13). |
...
| 39 | LOW | Inconsistency | `data_block_recovery.cpp:68-301` | Inconsistent error code conventions between `diagnose_slot` and `force_reset_slot` | ✅ FIXED 2026-03-11 — `DiagnoseError` enum added to `recovery_api.hpp`; all magic numbers in `diagnose_slot` and `diagnose_all_slots` replaced with enum constants; `kRecoveryErrorUnexpected` constant removed |
...
| 42 | MEDIUM | Error Handling | `producer_script_host.cpp:180-185` | Broker connection failure only warns — producer runs silently degraded | ✅ FIXED (pre-existing, confirmed 2026-03-12) — all 3 script hosts: `LOGGER_ERROR("[prod/cons/proc] broker connect failed; aborting")` + `return false`. The original "silently degraded" behaviour was eliminated during script host refactoring. Code verified at producer_script_host.cpp:262, consumer_script_host.cpp:187, processor_script_host.cpp:208. |
| 43 | MEDIUM | Resource Leak | `hub_producer.cpp:90-91` | `ctrl_send_queue` unbounded — no max depth or backpressure | ✅ FIXED (pre-existing) — `ctrl_queue_` is `MonitoredQueue<PendingCtrlSend>` with `max_depth=256`, drop-oldest policy, and backpressure monitoring via `on_warn`/`on_dead`/`on_cleared` callbacks |
| 44 | MEDIUM | Redundant Code | `producer_main.cpp` / `processor_main.cpp` / `consumer_main.cpp` | ~80% identical `main()`, arg parsing, init, keygen — no shared helpers | ✅ FIXED 2026-03-12 — HEP-0024 Phases 2, 5, 6: `role_cli.hpp` provides `parse_role_args()` / `resolve_init_name()` / `get_new_role_password()`; `RoleDirectory::create()` replaced all three `do_init()` blocks; `role_main_helpers.hpp` password helpers consolidated. All three mains migrated. |
...
| 46 | LOW | Missing Impl | `producer_config.cpp:129-143` | No validation for slot_count > 0, timeout_ms range, broker endpoint format | ✅ FIXED 2026-03-07 — `interval_ms > 0` (producer), `timeout_ms >= -1` (all three configs), `shm_slot_count > 0` (producer/processor) validated at parse time with descriptive `std::runtime_error`. Broker endpoint format not checked (caught at connection time). 7 new tests across producer/consumer/processor config suites. |
| 47 | LOW | Error Handling | `producer_script_host.cpp:377-381` | `thread.join()` with no timeout — hangs forever if thread is stuck | ACCEPTED 2026-03-11 — timed join is not safe here: detach fallback would cause use-after-free on the Impl data; shutdown always sets running_threads=false + ETERM which unblocks threads within ms. Comment added to all 3 script hosts. |
