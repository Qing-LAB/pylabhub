...
**Deferred:** Document the reconnect=re-register invariant in HEP-CORE-0007.
...
**File:** `connection_policy.hpp:54, 62`
...
**Recommendation:** Document lifetime requirements for `on_ready` in the header comment, or
switch to `std::shared_ptr`-based capture in examples.
...
### 3.3 HEP-CORE-0002 §3.3 state machine still wrong — ⚠️ DEFERRED (documented)
**Status:** MEMORY.md §"HEP + Doc Structure" already notes: "HEP-CORE-0002 §3.3 state
machine was WRONG — fixed in HEP-CORE-0007." Adding an inline redirect note to HEP-CORE-0002
§3.3 is tracked as a doc cleanup item.
...
| connection_policy.hpp   | N/A       | **Missing** | N/A     | N/A         | Missing EXPORT on structs      |
...
**2. LoopPolicy edge cases** (medium priority)
- Missing: MaxRate negative test (overrun_count = 0), zero interval, very large interval,
  metrics lifecycle across transactions, `TransactionContext::metrics()` reference identity,
  fresh-handle zero metrics
- **Recommendation:** Add 5-8 edge case tests to existing `test_datahub_loop_policy.cpp`
...
|----------|-------------------------------------------------------------|----------|
| **P0**   | Fix `std::_Exit()` -> `std::quick_exit()` (2 files)        | 5 min    |
| **P0**   | Fix memory_order_relaxed -> acquire in thread loops (2 files) | 10 min |
| **P0**   | Fix start_embedded() CAS ordering (2 files)                 | 5 min    |
| **P1**   | Add `PYLABHUB_UTILS_EXPORT` to KnownActor/ChannelPolicy     | 2 min    |
| **P1**   | Check file write errors in do_init() (2 files)              | 5 min    |
| **P1**   | Create test_datahub_connection_policy.cpp (5-8 tests)       | 2-3 hrs  |
| **P1**   | Fix connection_policy_from_str() silent fallback             | 10 min   |
| **P2**   | Add getpass() Windows fallback or platform guard             | 30 min   |
| **P2**   | Fix variable naming in hub_config.cpp                        | 5 min    |
| **P2**   | Extend test_datahub_loop_policy.cpp (+5-8 edge case tests)  | 1-2 hrs  |
| **P2**   | Archive HEP-CORE-0005                                        | 5 min    |
| **P2**   | Update HEP-CORE-0009 S2.6.2 status                          | 10 min   |
| **P2**   | Fix HEP-CORE-0002 S3.3 state machine reference              | 5 min    |
| **P3**   | Standardize test counts across docs                          | 15 min   |
| **P3**   | Document LoopPolicy vs LoopTimingPolicy distinction          | 15 min   |
| **P3**   | Document Messaging Facade as internal/frozen                 | 10 min   |
| **P3**   | Update TESTING_TODO.md phase inventory                       | 15 min   |
