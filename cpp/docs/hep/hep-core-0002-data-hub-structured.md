...
#### 10.1.1 Recommended Per-Slot Metadata Pattern
...
#### 10.1.2 Schema Negotiation via Broker

**Problem:** Producer and consumer must agree on payload structure.

**Solution:** Broker stores schema hash during registration; consumer validates before attach.

**Registration (Producer):**

```cpp
// Compute schema hash (SHA-256 of struct definition or IDL)
uint8_t schema_hash[32] = compute_schema_hash("SensorData_v2.proto");

MessageHub hub;
hub.connect("tcp://localhost:5555", server_public_key);

nlohmann::json reg_payload = {
    {"channel_name", "sensor_temperature"},
    {"shm_name", "datablock_sensor_temp_12345"},
    {"policy", "Single"},
    {"unit_block_size", 4096},
    {"ring_buffer_capacity", 1},
    {"shared_secret", encode_base64(secret_bytes, 8)},
    {"schema_hash", encode_base64(schema_hash, 32)},
    {"schema_version", "2.0"}
};

auto response = hub.send_request("PYLABHUB_REG_REQ", reg_payload, 5000);
...
```cpp
nlohmann::json disc_payload = {
    {"channel_name", "sensor_temperature"},
    {"client_id", "consumer_ui_001"}
};

auto response = hub.send_request("PYLABHUB_DISC_REQ", disc_payload, 5000);

// Validate schema match
std::string schema_hash_b64 = response["schema_hash"];
uint8_t schema_hash[32];
decode_base64(schema_hash_b64, schema_hash, 32);

uint8_t expected_hash[32] = compute_schema_hash("SensorData_v2.proto");
if (std::memcmp(schema_hash, expected_hash, 32) != 0) {
    throw std::runtime_error("Schema mismatch: producer and consumer incompatible");
}

// Proceed to attach DataBlock
std::string shm_name = response["shm_name"];
uint64_t shared_secret = decode_base64_to_uint64(response["shared_secret"]);
auto consumer = find_datablock_consumer(hub, "sensor_temperature", shared_secret);
```

**Benefits:**
- **Explicit Validation:** Schema mismatch detected at attach time (fail-fast)
- **Versioning:** Support multiple schema versions via `schema_version` field
- **Broker Enforcement:** Broker can reject registrations with duplicate channel + incompatible schema

---

### 10.2 Tier 1: Primitive API (Expert Mode)
...
public:
    ...
    // Statistics
    uint64_t get_commit_index() const;
    uint64_t get_write_index() const; // May be ahead of commit_index
};
```
...
public:
    ...
    // Seek operations
    void seek_latest(); // Jump to commit_index
...
### 10.4 Tier 3: Script Bindings (Productivity Mode)
...
uint32_t slot_index = static_cast<uint32_t>(write_index % ring_buffer_capacity);
SlotRWState* rw_state = &slot_rw_states[slot_index];

// Step 2: Wait for readers to drain (if wrapping around)
if (write_index >= ring_buffer_capacity) {
...
// Step 4: Transition to WRITING state
...
uint32_t prev_count = rw_state->reader_count.fetch_add(1, std::memory_order_acquire);

// Step 5: Double-check slot state (TOCTTOU mitigation)
if (rw_state->slot_state.load(std::memory_order_acquire) != SlotState::COMMITTED) {
    rw_state->reader_count.fetch_sub(1, std::memory_order_release);
    return nullptr; // Slot state changed during acquisition
}

// Step 6: Read data (zero-copy via std::span)
std::span<const std::byte> data = get_slot_buffer(slot_index);

// ... Process data ...

// Step 7: Decrement reader_count on handle destruction
rw_state->reader_count.fetch_sub(1, std::memory_order_release);
```
...
Latest-value, overwrite on write.

### 13.2 DoubleBuffer Policy

Front/back swap on commit.

### 13.3 RingBuffer Policy

FIFO queue; producer blocks when full; consumer blocks when empty.
...
### 14.1 Single-Block Layout (Detailed Memory Map)
...
graph TB
    subgraph SharedMemory["Shared Memory Segment (Single Block)"]
        ...
        RWState["Per-Slot RW State Array<br/>───────────────<br/>Size: ring_buffer_capacity × 48 bytes<br/>• write_lock (8B atomic)<br/>• reader_count (4B atomic)<br/>• write_generation (8B atomic)<br/>• slot_state (1B atomic)<br/>• writer_waiting (1B atomic)<br/>• padding[26] (26B)"]
...
### 14.3 Per-Slot Reader/Writer State Structure

For multi-reader support and safe wrap-around in ring buffers, each slot has dedicated coordination state:
...
struct SlotRWState {
    ...
    uint8_t padding[26];                     // Align to 48 bytes (avoid false sharing)
};
```
...
**Performance Impact:**
- Per-slot state adds `48 × N` bytes overhead
...
### 14.5 Total Size Calculation

```cpp
size_t total_size = 
    sizeof(SharedMemoryHeader) +                          // ~400 bytes
    ring_buffer_capacity * sizeof(SlotRWState) +          // N × 48 bytes
...
| `PYLABHUB_DB_NOTIFY` | Data ready notification |
| `PYLABHUB_ATTACH_ACK` | Consumer attach success |
| `PYLABHUB_ATTACH_NACK` | Consumer attach failure |
| `PYLABHUB_READY_NOTIFY` | Producer ready/epoch |
| `PYLABHUB_HB_REQ` | Heartbeat |
| `PYLABHUB_CONS_DROP` | Consumer timeout |
| `PYLABHUB_DETACH` | Consumer detach |

### 15.2 Notification Beyond Polling

Optional **DataPlane notifier** for low-latency local waits:

- Linux: futex
- POSIX: pshared condvar
- Windows: named event
...
## 17. Undecided Options and Alternatives

| Topic | Option A | Option B | Trade-Off |
|-------|----------|----------|-----------|
| **Slot coordination** | Exclusive spinlock | RW slot state | RW enables multi-reader |
| **Notification** | Poll commit_index | OS event/futex | Event reduces CPU |
| **Schema storage** | Flexible zone | Per-slot header | Header is explicit |
| **Secret distribution** | Broker (CurveZMQ) | Out-of-band config | Broker more secure |
...
%%{init: {'theme': 'dark'}}%%
graph TB
    subgraph Critical["🔴 Critical Priority (P1-P3) - Production Blockers"]
        P1["P1: Ring Buffer Policy Incomplete<br/>───────────<br/>• Impact: RingBuffer unusable<br/>• Risk: Data corruption, deadlock<br/>• Fix: 1-2 days"]
        P2["P2: owner_thread_id Race Condition<br/>───────────<br/>• Impact: Spinlock corruption<br/>• Risk: False ownership claims<br/>• Status: VERIFIED FIXED (already atomic)"]
        P3["P3: MessageHub Not Thread-Safe<br/>───────────<br/>• Impact: Multi-threaded use crashes<br/>• Risk: ZeroMQ socket corruption<br/>• Fix: 1 day (add mutex)"]
    end
    
    subgraph High["🟡 High Priority (P4-P6) - Functional Gaps"]
        P4["P4: Checksum Policy Not Enforced<br/>───────────<br/>• Impact: Manual checksums error-prone<br/>• Risk: Undetected corruption<br/>• Fix: 1 day"]
        P5["P5: Broker Integration Stub<br/>───────────<br/>• Impact: No discovery/notifications<br/>• Risk: Manual coordination required<br/>• Fix: 2 weeks"]
        P6["P6: Consumer Count Stale on Crash<br/>───────────<br/>• Impact: active_consumer_count wrong<br/>• Risk: Resource leak (minor)<br/>• Fix: Heartbeat-based auto-decrement"]
    end
    
    style Critical fill:#4a1a1a,stroke:#ff6b6b,color:#e0e0e0
    style High fill:#4a3a1a,stroke:#ffa500,color:#e0e0e0
```
