...
-   **Dynamic Unloading (`unload_module`)**:
    -   **Logic**: Uses a `RecursionGuard` and locks the graph. It decrements the module's reference count. If the count reaches zero, it runs the `shutdown()` callback and then recursively calls unload on its dependencies.
