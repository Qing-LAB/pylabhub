# py-demo-single-processor-shm — Single-Hub SHM Pipeline Demo

Single-hub, **pure SHM** pipeline: producer → processor → consumer, all on one
hub, all using shared-memory transport. This is the simplest pylabhub pipeline
configuration and the best starting point for new users.

## Pipeline

```
DemoProducer  ──[SHM]──▶  DemoDoubler  ──[SHM]──▶  DemoConsumer
(producer)    lab.demo.counter  (processor)  lab.demo.processed  (consumer)
```

4 processes (1 hub + 3 roles), all on the same host, all using SHM data transport.
ZMQ is used only for the control plane (broker registration, heartbeats, metrics).
...
bash cpp/share/py-demo-single-processor-shm/run_demo.sh
```

## Directory layout

```
share/py-demo-single-processor-shm/
├── hub/
...
2. Re-run `bash share/py-demo-single-processor-shm/run_demo.sh`.  No recompile needed.
...
```text
share/py-demo-single-processor-shm/hub/schemas/lab/demo/counter.v1.json
share/py-demo-single-processor-shm/hub/schemas/lab/demo/processed.v1.json
```
