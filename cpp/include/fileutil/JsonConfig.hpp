...
- Exception safety:
    - Public functions swallow exceptions and log errors where appropriate, returning bool for failure.
    - Templates follow the existing code style: read helpers use noexcept and return success/failure.

Notes for maintainers
---------------------
- If you need synchronous set-and-persist semantics, add a helper set_and_save() that calls set() then save().
...
// Define thread_local storage symbol here only as a declaration. The storage instance is defined in .cpp.
...
bool JsonConfig::with_json_write(F &&fn)
{
    ...
    try {
        ...
        // so caller can handle it — but this matches previous behavior where callback exceptions are allowed.
        throw;
    }
...
// ---------------- with_json_read ----------------
template <typename F>
bool JsonConfig::with_json_read(F &&cb) const noexcept
{
    try {
