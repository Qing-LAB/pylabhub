...
if(FORCE_USE_CLANG_ON_APPLE AND CMAKE_HOST_SYSTEM_NAME STREQUAL "Darwin")
  ...
  if(NOT CMAKE_GENERATOR STREQUAL "Xcode")
    ...
    if(_xcode_select_res EQUAL 0 AND EXISTS "${_clang_path_from_select}" AND EXISTS "${_clangpp_path_from_select}")
      ...
      message(STATUS "[pylabhub-toolchain] Forcing clang/clang++ via xcode-select on macOS: C=${_clang_path_from_select}, CXX=${_clangpp_path_from_select}")
    else()
      message(WARNING "[pylabhub-toolchain] Could not find clang/clang++ via xcode-select. Falling back to xcrun.")
      ...
      if(_xcrun_res_c EQUAL 0 AND _xcrun_res_cxx EQUAL 0 AND EXISTS "${_clang_path}" AND EXISTS "${_clangpp_path}")
        ...
        message(STATUS "[pylabhub-toolchain] Forcing clang/clang++ via xcrun on macOS: ${_clang_path}")
      else()
        message(WARNING "[pylabhub-toolchain] Could not find clang/clang++ via xcrun. Falling back to find_program.")
        ...
        if(_clang_path AND _clangpp_path)
          set(CMAKE_C_COMPILER "${_clang_path}" CACHE FILEPATH "C compiler" FORCE)
          set(CMAKE_CXX_COMPILER "${_clangpp_path}" CACHE FILEPATH "C++ compiler" FORCE)
          message(STATUS "[pylabhub-toolchain] Forcing clang/clang++ on macOS: ${_clang_path}")
        else()
          message(FATAL_ERROR "[pylabhub-toolchain] Clang not found on PATH. Consider installing Xcode or CommandLineTools.")
        endif()
      endif()
    endif()
  ...
  # unset environment variables that might confuse `xcodebuild` at build time.
    ...
    unset(ENV{CC})
    unset(ENV{CXX})
    message(STATUS "[pylabhub-toolchain] Xcode generator: Unsetting CC and CXX environment variables to allow default toolchain selection.")
  endif()
endif()