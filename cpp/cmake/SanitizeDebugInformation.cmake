...
function(_is_under prefix candidate out_var)
  ...
  set(_prefix_to_check "${prefix}")
  set(_candidate_to_check "${candidate}")
  if(WIN32)
    string(TOLOWER "${_prefix_to_check}" _prefix_to_check)
    string(TOLOWER "${_candidate_to_check}" _candidate_to_check)
  endif()

  string(LENGTH "${_prefix_to_check}" _plen)
  string(LENGTH "${_candidate_to_check}" _clen)
  if(_plen EQUAL 0 OR _clen LESS _plen)
    set(${out_var} FALSE PARENT_SCOPE)
    return()
  endif()
  string(SUBSTRING "${_candidate_to_check}" 0 ${_plen} _sub)
  if(NOT _sub STREQUAL "${_prefix_to_check}")
    set(${out_var} FALSE PARENT_SCOPE)
    return()
  endif()
  ...
  string(SUBSTRING "${_candidate_to_check}" ${_plen} 1 _next_char)
...
function(print_list_pairs msg list1_var list2_var)
  ...
  foreach(i RANGE 0 ${last})
      ...
      message(VERBOSE "    -- ${item1} [as ${item2}]")
  endforeach()
endfunction()
...
message(STATUS "[pylabhub-debuginfo-sanitize] Setting up debug information sanitization.")
...
message(VERBOSE "[pylabhub-debuginfo-sanitize] canonical CMAKE_SOURCE_DIR = ${_canon_source}")
message(VERBOSE "[pylabhub-debuginfo-sanitize] canonical CMAKE_BINARY_DIR = ${_canon_binary}")
...
if(CMAKE_CXX_COMPILER)
  ...
  if(_cxx_root AND NOT "${_cxx_root}" STREQUAL "")
    ...
    if(_p)
      ...
      message(VERBOSE "[pylabhub-debuginfo-sanitize] canonical TOOLCHAIN = ${_p}")
    endif()
  endif()
endif()
...
if(DEFINED CMAKE_SYSROOT AND NOT "${CMAKE_SYSROOT}" STREQUAL "")
  ...
  if(_p)
    ...
    message(VERBOSE "[pylabhub-debuginfo-sanitize] canonical SYSROOT = ${_p}")
  endif()
endif()
...
if(DEFINED ENV{CONDA_PREFIX} AND NOT "$ENV{CONDA_PREFIX}" STREQUAL "")
  ...
  if(_p)
    ...
    message(VERBOSE "[pylabhub-debuginfo-sanitize] canonical CONDA = ${_p}")
  endif()
endif()
...
if(DEFINED ENV{VCToolsInstallDir} AND NOT "$ENV{VCToolsInstallDir}" STREQUAL "")
  ...
  if(_p)
    ...
    message(VERBOSE "[pylabhub-debuginfo-sanitize] canonical VCTOOLS = ${_p}")
  endif()
endif()
...
print_list_pairs("[pylabhub-debuginfo-sanitize] The following directories will be examined:" _paths _origins)
message(STATUS "")
...
function(process_candidate final_paths_var final_origins_var candidate_path candidate_origin)  
  ...
  # message(VERBOSE " --** examining ${candidate_path} (origin=${candidate_origin})")
  ...
  set(_k 0)
  # message(VERBOSE "${_flen} ${_k}")
  while(_k LESS _flen)
    list(GET _fpaths ${_k} _kp)
    list(GET _forigs ${_k} _ko)
    # message(VERBOSE "      ** evaluating ${_kp} (origin=${_ko}), wild_card is ${_has_wildcard}")
    ...
    if(_origin_match)
      ...
      if(_under)
        ...
        list(LENGTH _fpaths _flen)
        # message(VERBOSE "                          ... removed...")
        continue()
      endif()
    endif()

    math(EXPR _k "${_k} + 1")
    # message(VERBOSE "                          ... moving on ...")
  endwhile()
...
while(_remaining_len GREATER 0)
  ...
  # message(VERBOSE "  remaining length: ${_remaining_len}")
  # print_list_pairs("[before processing, current _final_paths is]" _final_paths _final_origins)
  # message(VERBOSE "  continue to process: ${_pp} (origin=${_oo})")
  # message(VERBOSE "")
  ...
  elseif("${_oo}" STREQUAL "VCTOOLS")
    # remove any entries under this VCTOOLS prefix, then append VCTOOLS
    process_candidate(_shrink_paths _shrink_origins "${_pp}" "${_oo}" "*")
    list(APPEND _final_paths "${_pp}")
    list(APPEND _final_origins "${_oo}")
    continue()
  ...
  foreach(_kp IN LISTS _shrink_paths)
    ...
    if(_u2)
      set(_is_parent TRUE)
      # message(VERBOSE "   [detected] ${_pp} is the parent of ${_kp}, skipping")
      break()
    endif()
  endforeach()
...
while(_ilen GREATER 0)
  ...
  foreach(_pos RANGE 0 ${_ilen})
    ...
    # message(VERBOSE " ** currently shortest length is ${_shortest_len}")
    string(LENGTH "${_ipath}" _l)
    # message(VERBOSE " ** examining ${_ipath}, len is: ${_l}")
  ...
  # message(VERBOSE " ** chosen one is ${_chosen}")

  list(APPEND _emit_order "${_chosen}")
  list(REMOVE_AT _idxs ${_best_pos})
  # message(VERBOSE "  ** current _emit_order is ${_emit_order}")
  # message(VERBOSE "  ** current _idxs is ${_idxs}")
  list(LENGTH _idxs _ilen)
endwhile()
...
if(_remap_flags)
  ...
  if(CMAKE_CXX_COMPILER_ID MATCHES "GNU|Clang")
    ...
    message(VERBOSE "[pylabhub-debuginfo-sanitize] Will pass file/debug prefix map flags:")
  ...
  message(VERBOSE "[pylabhub-debuginfo-sanitize] No remap flags generated.")
endif()

message(STATUS "[pylabhub-debuginfo-sanitize] Debug information sanitization setup complete.")
message(STATUS "")
