...
#if defined(_WIN32)
    char buf[MAX_PATH];
    if (GetTempPathA(MAX_PATH, buf)) return std::string(buf);
    return ".\\";
#else
...
// Worker mode: each worker will attempt to write a unique marker into the config via with_json_write + save.
// Return code 0 indicates it succeeded in saving (i.e., acquisition of cross-process lock and atomic write).
static int worker_mode(const std::string &cfgpath, const std::string &worker_id)
{
    try {
        JsonConfig cfg(cfgpath);
        // Try to perform a quick write under with_json_write; if another process holds the file lock,
        // the atomic save may fail (non-blocking FileLock in save_locked). We return 0 only on success.
...
static bool basic_functional_tests(const fs::path &cfgfile)
{
    ...
    if (!cfg.set("int_val", 42)) { std::cerr << "set failed\n"; return false; }
    try {
        int v = cfg.get<int>("int_val");
        if (v != 42) { std::cerr << "get returned wrong value\n"; return false; }
    } catch (...) {
        std::cerr << "get threw\n"; return false;
    }

    int v2 = cfg.get_or<int>("nonexistent", 7);
    if (v2 != 7) { std::cerr << "get_or returned wrong default\n"; return false; }

    if (!cfg.has("int_val")) { std::cerr << "has failed\n"; return false; }

    if (!cfg.erase("int_val")) { std::cerr << "erase failed\n"; return false; }
    if (cfg.has("int_val")) { std::cerr << "erase did not remove key\n"; return false; }

    bool upd = cfg.update("obj", [](nlohmann::json &j){
        j["x"] = 100;
        j["s"] = "hello";
    });
    if (!upd) { std::cerr << "update failed\n"; return false; }

    try {
        auto j = cfg.get<nlohmann::json>("obj");
        if (!j.is_object() || j["x"] != 100) { std::cerr << "update content wrong\n"; return false; }
    } catch (...) {
...
static bool test_with_json_read(const fs::path &cfgfile)
{
    ...
    bool ok = cfg.with_json_read([&](const nlohmann::json &ref){
        if (!ref.is_object()) throw std::runtime_error("expected object");
        if (!ref.contains("k")) throw std::runtime_error("missing k");
        if (ref["k"] != "v") throw std::runtime_error("unexpected value");
    });
    if (!ok) { std::cerr << "with_json_read returned false\n"; return false; }
...
static bool test_with_json_write_and_save(const fs::path &cfgfile)
{
    ...
    if (!outer_ok) { std::cerr << "outer with_json_write failed\n"; return false; }
    ...
    if (!ok) { std::cerr << "with_json_write+save failed\n"; return false; }

    // read file to verify persistence
    std::string val = read_key_as_string(cfgfile, "inner");
    if (val != "yes") { std::cerr << "inner not persisted, val='" << val << "'\n"; return false; }

    std::cout << "[OK] with_json_write and save\n";
    return true;
}

// Test multithread behavior: thread A holds with_json_write and sleeps; thread B calls save() and must finish afterwards.
static bool test_multithread_save_ordering(const fs::path &cfgfile)
{
    ...
    std::thread tB([&](){
        ...
        while (!a_has.load()) std::this_thread::yield();
    ...
    tA.join();
    tB.join();
    if (!b_done.load()) { std::cerr << "threaded B did not complete\n"; return false; }
...
int main(int argc, char **argv)
{
    ...
    if (!basic_functional_tests(cfgfile)) return 1;
    if (!test_with_json_read(cfgfile)) return 2;
    if (!test_with_json_write_and_save(cfgfile)) return 3;
    if (!test_multithread_save_ordering(cfgfile)) return 4;

    // Multiprocess requires we know our exe path. argv[0] may be relative; use it directly.
    std::string exe_path = argv[0];
    if (!test_multiprocess(cfgfile, exe_path)) return 5;
