...
int main(int argc, char **argv) {
        ...
        if (dot_pos != std::string::npos) {
            ...
            if (module == "filelock") {
                ...
                if (scenario == "contention_log_access" && argc > 4) { // exe, mode, resource, log, iters
                    LifecycleGuard g();
                    return pylabhub::tests::worker::filelock::contention_log_access(argv[2], argv[3], std::stoi(argv[4]));
                }
    ...
    // These would likely be the tests designed for static lib, 
    // where no dynamic initialization is needed
    // Therefore we will not call explicitly call the LifecycleManager
