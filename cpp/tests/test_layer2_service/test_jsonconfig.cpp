 ...
 *
 * This fixture initializes the JsonConfig, FileLock, and Logger lifecycle modules
 * once for the entire suite (SetUpTestSuite/TearDownTestSuite), so each test can
 * use JsonConfig directly without spawning a subprocess.
 */
class JsonConfigTest : public ::testing::Test
{
  protected:
    ...
    {
        // Shut down lifecycle before removing the temp dir.
        s_lifecycle.reset();
        try
        {
...
std::unique_ptr<LifecycleGuard> JsonConfigTest::s_lifecycle;
